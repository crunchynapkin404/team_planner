...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS(f'{"Created" if created else "Updated"} Vacation leave type')
        )
        ...
        if not created:
            ...
            leave.description = 'Daytime leave (8-17h) - still available for waakdienst shifts'
            leave.save()
        
        self.stdout.write(
            self.style.SUCCESS(f'{"Created" if created else "Updated"} Leave leave type')
        )
        ...
        self.stdout.write(
            self.style.SUCCESS(f'{"Created" if created else "Updated"} Sick Leave leave type')
        )
        ...
        if not created:
            ...
            training.description = 'Training/Professional Development - available for emergency shifts'
            training.save()
        
        self.stdout.write(
            self.style.SUCCESS(f'{"Created" if created else "Updated"} Training leave type')
        )

        self.stdout.write(
            self.style.SUCCESS('Successfully configured all leave types with conflict handling!')
        )
