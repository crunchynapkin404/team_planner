...
class LeaveRequestForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            "days_requested": forms.NumberInput(
                attrs={"step": "0.5", "min": "0.5", "class": "form-control"},
            ),
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["days_requested"].help_text = _(
            "Total number of days requested (can be partial days)",
        )
    ...
    def clean(self):
        ...
        if start_date and end_date:
            ...
            if days_requested and days_requested > max_days:
                raise ValidationError(
                    _(
                        "Days requested ({}) cannot exceed the number of days in the date range ({}).",
                    ).format(days_requested, max_days),
                )
...
class LeaveRequestResponseForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["rejection_reason"].help_text = _(
            "Required if rejecting the request",
        )
...
class BulkLeaveApprovalForm(forms.Form):
    ...
    def clean(self):
        ...
        if action == "reject" and not bulk_rejection_reason:
            raise ValidationError(
                {
                    "bulk_rejection_reason": _(
                        "Bulk rejection reason is required when rejecting requests.",
                    ),
                },
            )
