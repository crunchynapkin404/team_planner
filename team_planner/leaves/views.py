...
from .forms import LeaveRequestResponseForm
from .forms import LeaveRequestSearchForm
...
class LeaveRequestListView(ListView):
        ...
        if form.is_valid():
            ...
            if form.cleaned_data.get("start_date_from"):
                queryset = queryset.filter(
                    start_date__gte=form.cleaned_data["start_date_from"],
                )
            if form.cleaned_data.get("start_date_to"):
                queryset = queryset.filter(
                    start_date__lte=form.cleaned_data["start_date_to"],
                )
    ...
    def get_context_data(self, **kwargs):
        ...
        context["search_form"] = LeaveRequestSearchForm(
        ...
        context["with_conflicts_count"] = len(
            [req for req in queryset if req.has_shift_conflicts],
        )
...
class LeaveRequestDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if context["conflicting_shifts"].exists():
            ...
            context["related_swap_requests"] = (
                SwapRequest.objects.filter(requesting_shift__in=shift_ids)
                .select_related("requesting_employee", "target_employee", "approved_by")
                .order_by("-created")
            )
...
def check_leave_conflicts_ajax(request):
    ...
    temp_request = LeaveRequest(
        employee=request.user, start_date=start_date, end_date=end_date,
    )
...
def leave_dashboard(request):
    ...
    if request.user.has_perm("leaves.change_leaverequest"):
        ...
        context["pending_with_conflicts"] = [
            req for req in context["pending_approvals"] if req.has_shift_conflicts
        ]
        context["pending_without_conflicts"] = [
            req for req in context["pending_approvals"] if not req.has_shift_conflicts
        ]
