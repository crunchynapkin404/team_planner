...
class LeaveType(TimeStampedModel):
    ...
    class ConflictHandling(models.TextChoices):
        ...
        DAYTIME_ONLY = (
            "daytime_only",
            _("Daytime Only - Blocks day shifts, available for waakdienst"),
        )
...
class LeaveRequest(TimeStampedModel):
    ...
    start_time = models.TimeField(
        ...
        blank=True,
        help_text=_(
            "Start time for partial day leave (uses leave type default if blank)",
        ),
    )
    end_time = models.TimeField(
        _("End Time"),
        ...
        blank=True,
        help_text=_(
            "End time for partial day leave (uses leave type default if blank)",
        ),
    )
    ...
    def get_conflicting_shifts(self):
        ...
        if (
            self.leave_type.conflict_handling
            == LeaveType.ConflictHandling.FULL_UNAVAILABLE
        ):
            return shifts_queryset

        # For daytime only (like leave), filter by shift type and time
        if (
            self.leave_type.conflict_handling == LeaveType.ConflictHandling.DAYTIME_ONLY
        ):
            # Get the effective start and end times for this leave
            leave_start_time = (
                self.start_time or self.leave_type.start_time or time(8, 0)
            )
            ...
            daytime_shifts = shifts_queryset.exclude(template__shift_type="waakdienst")
            ...
            for shift in daytime_shifts:
                ...
                if (
                    shift_start_time < leave_end_time
                    and shift_end_time > leave_start_time
                ):
    ...
    def get_suggested_swap_employees(self):
        ...
        available_employees = User.objects.filter(is_active=True).exclude(
            pk=self.employee.pk,
        )
        ...
        if self.is_within_active_planning_window():
            ...
            for shift in conflicting_shifts:
                if not SwapRequest.objects.filter(
                    requesting_shift=shift, status=SwapRequest.Status.APPROVED,
                ).exists():
                    return False
            return True
