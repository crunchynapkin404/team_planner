...
class LeaveRequestTestCase(TestCase):
    def setUp(self):
        ...
        self.employee1 = EmployeeProfile.objects.create(
            user=self.user1, employee_id="EMP001", hire_date=timezone.now().date(),
        )
        ...
        self.shift_template = ShiftTemplate.objects.create(
            name="Incidents Shift", shift_type="incidents", duration_hours=8,
        )
        ...
        self.shift1 = Shift.objects.create(
            ...
            start_datetime=timezone.make_aware(
                timezone.datetime.combine(date.today() + timedelta(days=5), time(9, 0)),
            ),
            end_datetime=timezone.make_aware(
                timezone.datetime.combine(date.today() + timedelta(days=5), time(17, 0)),
            ),
            status="confirmed",
        )
    ...
    def test_create_leave_request(self):
        ...
        assert leave_request.employee == self.user1
        assert leave_request.leave_type == self.leave_type
    ...
    def test_shift_conflict_detection(self):
        ...
        assert conflicting_shifts.count() == 1
        assert conflicting_shifts.first() == self.shift1
    ...
    def test_no_shift_conflicts(self):
        ...
        # Check no conflicts
        conflicting_shifts = leave_request.get_conflicting_shifts()
        assert conflicting_shifts.count() == 0
        assert not leave_request.has_shift_conflicts
    ...
    def test_suggested_swap_employees(self):
        ...
        assert self.user2 in suggested_employees
...
class LeaveRequestIntegrationTestCase(TestCase):
    def setUp(self):
        ...
        self.employee = EmployeeProfile.objects.create(
            user=self.user, employee_id="EMP001", hire_date=timezone.now().date(),
        )
        self.leave_type = LeaveType.objects.create(
            name="vacation", description="Vacation leave",
        )
    ...
    def test_leave_request_str_representation(self):
        ...
        assert str(leave_request) == expected_str
    ...
    def test_leave_request_status_choices(self):
        ...
        for status in valid_statuses:
            ...
            assert leave_request.status == status
    ...
    def test_leave_type_choices(self):
        ...
        leave_types = [
            "sick",
            "personal",
        ]  # Avoid 'vacation' which already exists in setUp
        ...
        for i, type_name in enumerate(leave_types):
            ...
            assert leave_request.leave_type.name == f"{type_name}_{i}"
