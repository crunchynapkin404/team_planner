...
from .forms import BulkSwapApprovalForm
from .forms import ShiftSearchForm
        ...
        if form.is_valid():
            if form.cleaned_data.get("start_date"):
                queryset = queryset.filter(
                    start_datetime__gte=form.cleaned_data["start_date"],
                )
            if form.cleaned_data.get("end_date"):
                queryset = queryset.filter(
                    end_datetime__lte=form.cleaned_data["end_date"],
                )
            if form.cleaned_data.get("shift_type"):
                queryset = queryset.filter(
                    template__shift_type=form.cleaned_data["shift_type"],
                )
            if form.cleaned_data.get("employee"):
                queryset = queryset.filter(
                    assigned_employee=form.cleaned_data["employee"],
                )
...
class ShiftDetailView(DetailView):
    ...
    def get_queryset(self):
        return Shift.objects.select_related(
            ...
            "time_entries", "swap_requests_as_source", "swap_requests_as_target",
        )
...
def create_swap_request(request, shift_pk):
    ...
    if shift.assigned_employee != request.user:
        messages.error(
            request, "You can only create swap requests for your own shifts.",
        )
    ...
    if request.method == "POST":
        form = SwapRequestForm(
            request.POST, requesting_employee=request.user, requesting_shift=shift,
        )
        if form.is_valid():
    ...
    return render(
        request,
        "shifts/create_swap_request.html",
        {
            "form": form,
            "shift": shift,
        },
...
def respond_to_swap_request(request, pk):
    ...
    if not _can_respond_to_swap(request.user, swap_request):
        return HttpResponseForbidden(
            "You don't have permission to respond to this swap request.",
        )
...
def bulk_swap_approval(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if success_count > 0:
                messages.success(
                    request, f"Successfully {action}d {success_count} swap request(s).",
                )
            if error_count > 0:
                messages.warning(
                    request, f"{error_count} swap request(s) could not be {action}d.",
                )
    ...
    return render(
        request,
        "shifts/bulk_swap_approval.html",
        {
            "form": form,
        },
    )
...
def shifts_api(request):
    ...
    queryset = Shift.objects.select_related("template", "assigned_employee").order_by(
        "start_datetime",
    )
    ...
    for shift in queryset:
        event = {
            ...
            "extendedProps": {
                ...
                "engineerName": shift.assigned_employee.name
                if shift.assigned_employee
                else "Unassigned",
                "engineerId": str(shift.assigned_employee.pk)
                if shift.assigned_employee
                else "",
...
def dashboard_api(request):
    ...
    if not user_teams:
        team_member_ids = User.objects.filter(is_active=True).values_list(
            "id", flat=True,
        )
    else:
        # Get all team members from the user's teams
        team_member_ids = TeamMembership.objects.filter(
            team_id__in=user_teams, is_active=True,
    ...
    total_engineers = User.objects.filter(
        id__in=team_member_ids, is_active=True,
    ).count()

    # Get engineers on leave today from the user's teams
    engineers_on_leave_today = (
...
def _can_respond_to_swap(user, swap_request):
    ...
    if hasattr(user, "employee_profile"):
        ...
        if user_teams.intersection(requesting_teams) or user_teams.intersection(
            target_teams,
        ):
            return True
