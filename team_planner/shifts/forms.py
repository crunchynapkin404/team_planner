...
class SwapRequestForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            "reason": forms.Textarea(attrs={"rows": 4, "placeholder": "Explain why you need this swap..."}),
        }
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.requesting_employee:
            self.fields["target_employee"].queryset = User.objects.filter(
                is_active=True
            ).exclude(pk=self.requesting_employee.pk).order_by("username")
        ...
        if target_shift and target_employee:
            if target_shift.assigned_employee != target_employee:
                raise forms.ValidationError(
                    "Selected target shift must belong to the target employee."
                )
...
class ShiftSearchForm(forms.Form):
    ...
    shift_type = forms.ChoiceField(
        choices=[("", "All Types")] + list(Shift.objects.values_list(
            "template__shift_type", "template__shift_type"
        ).distinct()),
        required=False,
...
class BulkSwapApprovalForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if user and not user.is_superuser:
            ...
            if hasattr(user, "employee_profile") and user.employee_profile.teams.exists():
                ...
                self.fields["swap_requests"].queryset = SwapRequest.objects.filter(
                    status="pending",
                    requesting_employee__in=team_members
                )
            else:
