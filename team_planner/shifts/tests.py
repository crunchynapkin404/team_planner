...
class SwapRequestTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.user1 = User.objects.create_user(
            username="engineer1", email="engineer1@example.com", password="testpass123",
        )
        self.user2 = User.objects.create_user(
            username="engineer2", email="engineer2@example.com", password="testpass123",
        )
        ...
        self.template = ShiftTemplate.objects.create(
            name="Test Incidents Shift", shift_type="incidents", duration_hours=40,
        )
    ...
    def test_create_swap_request(self):
        ...
        assert swap_request.requesting_employee == self.user1
        assert swap_request.target_employee == self.user2
    ...
    def test_swap_request_validation(self):
        ...
        assert "Cannot swap shift with yourself" in errors
    ...
    def test_swap_approval(self):
        ...
        assert swap_request.approved_by == self.manager
        assert swap_request.approved_datetime is not None

        # Check that shift was reassigned
        self.shift1.refresh_from_db()
        assert self.shift1.assigned_employee == self.user2
...
class LeaveShiftIntegrationTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.leave_type = LeaveType.objects.create(
            name="Annual Leave", default_days_per_year=25, is_paid=True,
        )
    ...
    def test_leave_request_conflict_detection(self):
        ...
        assert leave_request.has_shift_conflicts
        conflicting_shifts = leave_request.get_conflicting_shifts()
        assert self.shift in conflicting_shifts

        # Check that leave cannot be approved without resolving conflicts
        assert not leave_request.can_be_approved()

        # Check blocking message
        blocking_message = leave_request.get_blocking_message()
        assert blocking_message is not None
        assert "conflicts with this leave request" in blocking_message
    ...
    def test_leave_approval_after_swap(self):
        ...
        other_user = User.objects.create_user(
            username="other_engineer", email="other@example.com", password="testpass123",
        )
        ...
        assert leave_request.can_be_approved()

        # Verify shift was reassigned
        self.shift.refresh_from_db()
        assert self.shift.assigned_employee == other_user
...
class PhaseThreeIntegrationTestCase(TestCase):
    ...
    def test_phase_three_models_exist(self):
        ...
        assert hasattr(SwapRequest, "requesting_employee")
        assert hasattr(SwapRequest, "target_employee")
        assert hasattr(SwapRequest, "requesting_shift")
        assert hasattr(SwapRequest, "status")

        # Test SwapRequest methods
        assert hasattr(SwapRequest, "approve")
        assert hasattr(SwapRequest, "reject")
        assert hasattr(SwapRequest, "cancel")
        assert hasattr(SwapRequest, "validate_swap")

        # Test LeaveRequest enhancements
        assert hasattr(LeaveRequest, "get_conflicting_shifts")
        assert hasattr(LeaveRequest, "has_shift_conflicts")
        assert hasattr(LeaveRequest, "get_suggested_swap_employees")
        assert hasattr(LeaveRequest, "can_be_approved")
    ...
    def test_admin_interfaces_registered(self):
        ...
        assert Shift in admin.site._registry
        assert SwapRequest in admin.site._registry
        assert ShiftTemplate in admin.site._registry
