...
from team_planner.users.forms import UserProfileForm, RecurringLeavePatternInlineFormSet
...
class UserUpdateView(LoginRequiredMixin, SuccessMessageMixin, UpdateView):
    ...
    def form_valid(self, form):
        ...
        if recurring_patterns_formset.is_valid():
            ...
            from team_planner.employees.models import RecurringLeavePattern
            active_patterns = RecurringLeavePattern.objects.filter(employee=user_obj, is_active=True).count()
            if active_patterns == 0:
                messages.info(self.request, _("No recurring leave patterns are currently active. You're available for all shifts."))
            else:
                messages.success(self.request, _(f"Successfully updated {active_patterns} recurring leave pattern(s)."))
            
            return response
        else:
            messages.error(self.request, _("Please correct the errors in your recurring leave patterns."))
