...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            with transaction.atomic():
                ...
                engineers_created = self._create_engineers(
                    team, team_data["engineers"], dry_run,
                )
    ...
    def _create_shift_type_skills(self, dry_run=False):
        ...
        shift_types_to_create = [
            ...
            (
                "waakdienst",
                "Waakdienst - On-call/standby shifts (evenings, nights, weekends)",
            ),
        ]
        ...
        for skill_name, description in shift_types_to_create:
            if dry_run:
                ...
                skill, created = EmployeeSkill.objects.get_or_create(
                    name=skill_name, defaults={"description": description},
                )
                if created:
                    skills_created.append(skill_name)
    ...
    def _create_team(self, team_name, dry_run=False):
        ...
        department, _ = Department.objects.get_or_create(
            name="Engineering", defaults={"description": "Engineering Department"},
        )
        ...
        team, created = Team.objects.get_or_create(
            ...
            defaults={
                "description": f"{team_name} - Engineering team with incident and waakdienst capabilities",
            },
        )
        if created:
        ...
        for engineer_data in engineers_data:
            if dry_run:
                ...
                if not existing:
                    ...
                    self.stdout.write(
                        f"Engineer already exists: {engineer_data['username']}",
                    )
            else:
                # Create user
                ...
                profile, profile_created = EmployeeProfile.objects.get_or_create(
                    ...
                    defaults={
                        ...
                        "available_for_incidents": "incidents"
                        in engineer_data["shift_types"],
                        "available_for_waakdienst": "waakdienst"
                        in engineer_data["shift_types"],
                    },
                )
                ...
                if profile_created:
                    ...
                    profile.available_for_incidents = (
                        "incidents" in engineer_data["shift_types"]
                    )
                    profile.available_for_waakdienst = (
                        "waakdienst" in engineer_data["shift_types"]
                    )
                    profile.save()
                ...
                for shift_type in engineer_data["shift_types"]:
                    try:
                        ...
                        if skill not in profile.skills.all():
                            ...
                            self.stdout.write(
                                f"Added {shift_type} skill to {user.username}",
                            )
                    except EmployeeSkill.DoesNotExist:
                        self.stdout.write(
                            self.style.WARNING(
                                f'Skill "{shift_type}" not found for {user.username}',
                            ),
                        )
