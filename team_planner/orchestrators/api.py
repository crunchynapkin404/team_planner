...
from team_planner.orchestrators.anchors import WEEKDAY_FRI
from team_planner.orchestrators.anchors import WEEKDAY_MON
...
from team_planner.orchestrators.tasks import extend_rolling_horizon_task
...
from .fairness_calculators import (
    ComprehensiveFairnessCalculator as FairnessCalculator,
)  # Use comprehensive calculator for API
from .models import OrchestrationResult
from .models import OrchestrationRun
from .unified import UnifiedOrchestrator  # Legacy compatibility
...
def orchestrator_create_api(request):
    ...
    try:
        ...
        if not any(
            ...
            return Response(
                {"error": "At least one shift type must be selected"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        try:
            ...
            return Response(
                {"error": "Invalid team"}, status=status.HTTP_400_BAD_REQUEST,
            )
        tz = get_team_tz(team)
        ...
        if schedule_incidents or schedule_incidents_standby:
            first_monday = next_weekday_time(
                start_datetime, WEEKDAY_MON, 8, tz=tz, strictly_after=True,
            )
            # Friday 17:00 of that week
            monday_of_week_dt = first_monday.replace(
                hour=0, minute=0, second=0, microsecond=0,
            )
            first_friday = monday_of_week_dt + timedelta(
                days=(WEEKDAY_FRI - WEEKDAY_MON) % 7,
            )
            first_friday = first_friday.replace(
                hour=17, minute=0, second=0, microsecond=0,
            )
            adjusted_end = max(adjusted_end, first_friday)
        ...
        if schedule_waakdienst:
            ...
            first_waak_start = next_weekday_time(
                start_datetime, weekday, start_hour, tz=tz, strictly_after=True,
            )
            first_waak_end = (first_waak_start + timedelta(days=7)).replace(
                hour=end_hour, minute=0, second=0, microsecond=0,
            )
            adjusted_end = max(adjusted_end, first_waak_end)
        ...
        with transaction.atomic():
            ...
            try:
                ...
                if preview_only:
                    ...
                    run.incidents_standby_shifts_created = result.get(
                        "incidents_standby_shifts", 0,
                    )
                    ...
                    agg: defaultdict[tuple[str, date], dict[str, Any]] = defaultdict(
                        lambda: {
                    ...
                    for a in assignments:
                        st = a["shift_type"]
                        if (st == "incidents" and not schedule_incidents) or (
                            st == "waakdienst" and not schedule_waakdienst
                        ):
                            continue
                        ...
                        emp_key = a.get(
                            ...
                            a.get("assigned_employee_name", "unknown"),
                        )
                        agg[key]["counts"][emp_key] += 1  # type: ignore[index]
                        agg[key]["reason"] = (
                            a.get("assignment_reason") or "Fair distribution"
                        )
                    ...
                    return Response(
                        {
                            ...
                            "incidents_standby_shifts": result.get(
                                "incidents_standby_shifts", 0,
                            ),
                            ...
                            "potential_duplicates": result.get(
                            ...
                            + (
                                ...
                                if result.get("reassignment_summary", {}).get(
                ...
                run.completed_at = timezone.now()
                run.save()
                logger.error(f"Orchestration failed: {e!s}", exc_info=True)
                return Response(
        ...
        logger.error(f"API error: {e!s}", exc_info=True)
...
def orchestrator_apply_preview_api(request):
    ...
    if not (
        request.user.is_staff
        or request.user.has_perm("orchestrators.change_orchestrationrun")
    ):
        return Response(
            {"error": "Permission denied - staff access required"},
            status=status.HTTP_403_FORBIDDEN,
    ...
    try:
        ...
        run = OrchestrationRun.objects.get(
            pk=run_id, status=OrchestrationRun.Status.PREVIEW,
        )
        ...
        with transaction.atomic():
            ...
            shifts_created = 0
            for result in OrchestrationResult.objects.filter(
                orchestration_run=run, is_applied=False,
            ):
                if result.shift_type == "incidents":
                    ...
                    start_datetime = timezone.make_aware(
                        datetime.combine(
                            result.week_start_date, datetime.min.time().replace(hour=8),
                        ),
                    )
                    end_datetime = timezone.make_aware(
                        datetime.combine(
                            result.week_start_date + timedelta(days=4),
                    ...
                    start_datetime = timezone.make_aware(
                        datetime.combine(
                            result.week_start_date + timedelta(days=2),
                            datetime.min.time().replace(hour=17),
                        ),
                    )
                    end_datetime = timezone.make_aware(
                        datetime.combine(
                            result.week_start_date + timedelta(days=9),
        ...
        return Response(
            {"error": "Preview orchestration not found"},
...
def fairness_api(request):
    ...
    try:
        ...
        for employee in active_employees:
            ...
            available_incidents = (
                getattr(profile, "available_for_incidents", False) if profile else False
            )
            available_waakdienst = (
                getattr(profile, "available_for_waakdienst", False)
                if profile
                else False
            )
            hours_per_week = (
                availability_info["hours_per_week"]
                if (available_incidents or available_waakdienst)
                else 0.0
            )
            base_assignments[employee.pk] = {
                ...
                "availability_percentage": availability_info["percentage"]
                if hours_per_week > 0
                else 0.0,
        ...
        total_standby = sum(
            v.get("incidents_standby", 0.0) for v in assignments.values()
        )
        total_waakdienst = sum(v.get("waakdienst", 0.0) for v in assignments.values())
        cap_incidents = sum(
            v.get("available_hours_per_week", 0.0)
            for v in assignments.values()
            if v.get("available_incidents")
        )
        cap_standby = cap_incidents  # same eligibility
        cap_waakdienst = sum(
            v.get("available_hours_per_week", 0.0)
            for v in assignments.values()
            if v.get("available_waakdienst")
        )
        ...
        avg_incidents = (
            (
                sum(v.get("incidents", 0.0) for v in assignments.values())
                / total_employees
            )
            if total_employees
            else 0.0
        ...
        avg_waakdienst = (
            (
                sum(v.get("waakdienst", 0.0) for v in assignments.values())
                / total_employees
            )
            if total_employees
            else 0.0
        ...
        avg_total = (
            (
                sum(
                    (
                        v.get("incidents", 0.0)
                        + v.get("incidents_standby", 0.0)
                        + v.get("waakdienst", 0.0)
                    )
                    for v in assignments.values()
                )
                / total_employees
        ...
        for employee in active_employees:
            ...
            # Expected per type
            def exp_dev_fair(
                assigned: float, cap_total: float, type_total: float, eligible: bool,
            ):
            ...
            exp_i, dev_i, fair_i = exp_dev_fair(
                inc_hours, cap_incidents, total_incidents, avail_i,
            )
            exp_s, dev_s, fair_s = exp_dev_fair(
                stb_hours, cap_standby, total_standby, avail_i,
            )
            exp_w, dev_w, fair_w = exp_dev_fair(
                waa_hours, cap_waakdienst, total_waakdienst, avail_w,
            )

            # Overall fairness = average of available type fair scores
            fair_vals = [
                v for v in (fair_i, fair_s, fair_w) if isinstance(v, (int, float))
            ]
            overall_fairness = (
                round(sum(fair_vals) / len(fair_vals), 2) if fair_vals else None
            )

            # Last assignment date (from applied orchestration results)
            last_assignment = (
                OrchestrationResult.objects.filter(employee=employee, is_applied=True)
                .order_by("-applied_at")
                .first()
            )
            ...
            try:
                ...
                full_name = ""
            username = (
                getattr(employee, "username", "")
                or getattr(employee, "email", "")
                or f"User {getattr(employee, 'pk', '')}"
            )
            ...
            row = {
                ...
                if overall_fairness is not None
                else 0.0,  # legacy key
                ...
                "available_incidents": available_incidents,
                "available_waakdienst": available_waakdienst,
                "last_assignment_date": last_assignment.applied_at.date().isoformat()
                if last_assignment and last_assignment.applied_at
                else None,
            }
        ...
        employee_data.sort(
            key=lambda x: (
                x.get("overall_fairness") is None,
                x.get("overall_fairness") or 0,
            ),
        )
        ...
        logger.error(f"Fairness API error: {e!s}", exc_info=True)
        return Response({"error": str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
def orchestrator_run_horizon_api(request):
    ...
    try:
        ...
        shift_types = payload.get(
            "shift_types",
        )  # optional list: ['incidents','waakdienst','incidents_standby']
        ...
        if shift_types is not None and not isinstance(shift_types, (list, tuple)):
            return Response(
                {"error": "shift_types must be a list of strings"},
                status=status.HTTP_400_BAD_REQUEST,
            )

        summary = extend_rolling_horizon_core(
            months=months,
            dry_run=dry_run,
            team_ids=team_ids,
            weeks=weeks,
            shift_types=list(shift_types) if shift_types is not None else None,
        )
...
def orchestrator_run_horizon_async_api(request):
    ...
    try:
        team_ids_param = (
            [*team_ids] if isinstance(team_ids, (list, tuple)) else team_ids
        )
        shift_types_param = (
            [*shift_types] if isinstance(shift_types, (list, tuple)) else None
        )
        async_result = extend_rolling_horizon_task.delay(
            months=months,
            dry_run=dry_run,
            team_ids=team_ids_param,
            weeks=weeks,
            shift_types=shift_types_param,
        )  # type: ignore
...
def orchestrator_enable_auto_api(request):
    ...
    try:
        ...
        return Response(
            {"error": "django-celery-beat not installed"},
...
def orchestrator_auto_overview_api(request):
    ...
    try:
        now = timezone.now()
        require_seed = bool(
            getattr(settings, "ORCHESTRATOR_AUTO_ROLL_REQUIRES_SEED", True),
        )
            ...
            for pt in beat_tasks:
                ...
                if len(parts) == 3:
                    ...
                    try:
                        ...
                        continue
                    per_team_type_enabled.setdefault(team_id_int, {})[shift_type] = (
                        bool(pt.enabled)
                    )
        except Exception:
            beat = None
        ...
        for team in qs:
            ...
            coverage_end = (
                coverage_qs.order_by("-end_datetime")
...
def orchestrator_auto_toggle_api(request):
    ...
    try:
        ...
        if "team_id" not in payload or "shift_type" not in payload:
            return Response(
                {"error": "team_id and shift_type are required"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        try:
        ...
        if shift_type not in ("incidents", "incidents_standby", "waakdienst"):
            return Response(
                {"error": "Invalid shift_type"}, status=status.HTTP_400_BAD_REQUEST,
            )
        # Ensure team exists
        ...
        task_name = (
            f"orchestrators.extend_rolling_horizon_nightly:{team_id}:{shift_type}"
        )
        weeks = int(getattr(settings, "ORCHESTRATOR_ROLLING_WEEKS", 26))
        kwargs = {
            "weeks": weeks,
        ...
        if not created:
            ...
            pt.save()
        return Response(
            {"team_id": team_id, "shift_type": shift_type, "enabled": pt.enabled},
            status=status.HTTP_200_OK,
        )
    except Team.DoesNotExist:
        return Response(
            {"error": "Invalid team_id"}, status=status.HTTP_400_BAD_REQUEST,
        )
    except ImportError:
        return Response(
            {"error": "django-celery-beat not installed"},
