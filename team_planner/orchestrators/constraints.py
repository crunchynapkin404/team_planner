...
class AvailabilityResult:
    ...
    is_available: bool
    reason: str = ""
    conflicting_leave: Any | None = None
    conflicting_shift: Any | None = None
...
class BaseConstraintChecker:
    ...
    def _get_shift_types(self) -> list[str]:
        ...
        msg = "Subclasses must implement _get_shift_types"
    ...
    def _get_relevant_time_range(self) -> tuple[time, time]:
        ...
        msg = "Subclasses must implement _get_relevant_time_range"
        raise NotImplementedError(msg)
    ...
    def _get_relevant_days(self) -> list[int]:
        ...
        msg = "Subclasses must implement _get_relevant_days"
        raise NotImplementedError(msg)
    ...
    def is_employee_available_for_shift_type(self, employee: Any) -> bool:
        ...
        try:
            ...
            if ShiftType.WAAKDIENST in shift_types:
    ...
    def check_approved_leave_conflicts(
        self, employee: Any, check_date: date,
    ) -> AvailabilityResult:
        """Check if employee has approved leave on this date."""
        # Convert date to datetime range for comparison
        timezone.make_aware(datetime.combine(check_date, time.min))
        timezone.make_aware(datetime.combine(check_date, time.max))
    ...
    def check_existing_shift_conflicts(
        self, employee: Any, check_date: date,
    ) -> AvailabilityResult:
    ...
    def check_employee_availability(
        ...
        if check_date.weekday() not in self._get_relevant_days():
            return AvailabilityResult(
                is_available=False, reason="Not a relevant day for this shift type",
            )
    ...
    def check_recurring_leave_conflicts(
        self, employee: Any, check_date: date,
    ) -> AvailabilityResult:
        """Check recurring leave patterns. Override in subclasses for specific time checking."""
        msg = "Subclasses must implement check_recurring_leave_conflicts"
        raise NotImplementedError(
            msg,
        )

    def get_available_employees(
        self, employees: list[Any], check_date: date,
    ) -> list[Any]:
    ...
    def get_unavailable_employees_with_reasons(
        self, employees: list[Any], check_date: date,
    ) -> dict[Any, str]:
...
class IncidentsConstraintChecker(BaseConstraintChecker):
    ...
    def check_recurring_leave_conflicts(
        ...
        logger.debug(
            f"Allowing incidents assignment for {employee.username} on {check_date} - conflicts will be handled by reassignment",
        )
        return AvailabilityResult(is_available=True)
...
class WaakdienstConstraintChecker(BaseConstraintChecker):
    ...
    def check_recurring_leave_conflicts(
        ...
        ).filter(Q(effective_until__isnull=True) | Q(effective_until__gte=check_date))
        ...
        for pattern in patterns:
            ...
            if pattern.applies_to_date(check_date):
                ...
                if affected_hours:
                    ...
                    if (
                        pattern.coverage_type
                        == RecurringLeavePattern.CoverageType.FULL_DAY
                    ):
                        return AvailabilityResult(
                            is_available=False,
                    ...
                    affected_hours["start_datetime"].time()
                    affected_hours["end_datetime"].time()

                    # If pattern affects afternoon (12:00-17:00) or extends into evening, it might conflict
                    if (
                        pattern.coverage_type
                        == RecurringLeavePattern.CoverageType.AFTERNOON
                    ):
                        ...
                        logger.debug(
                            f"Afternoon pattern {pattern.name} ends at Waakdienst start time - allowing",
                        )
...
class ConstraintCheckerFactory:
    ...
    @staticmethod
    def create_incidents_checker(
        start_date: datetime, end_date: datetime,
    ) -> IncidentsConstraintChecker:
        """Create an IncidentsConstraintChecker."""
        return IncidentsConstraintChecker(start_date, end_date)

    @staticmethod
    def create_waakdienst_checker(
        start_date: datetime, end_date: datetime,
    ) -> WaakdienstConstraintChecker:
