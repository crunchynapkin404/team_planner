...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            f"Testing orchestrator for period: {start_date} to {end_date}",
        )
        ...
        test_user, _ = User.objects.get_or_create(
            username="test_orchestrator", defaults={"name": "Test Orchestrator User"},
        )
        ...
        try:
            ...
            try:
                ...
                self.stdout.write(
                    self.style.ERROR("Team 7 not found. Please ensure Team 7 exists."),
                )
                return
            ...
            if preview_only:
                ...
                self.stdout.write(
                    f"  Total shifts planned: {result.get('total_shifts', 0)}",
                )
                self.stdout.write(
                    f"  Incident shifts: {result.get('incident_shifts', 0)}",
                )
                self.stdout.write(
                    f"  Waakdienst shifts: {result.get('waakdienst_shifts', 0)}",
                )
                ...
                if "planned_shifts" in result:
                    ...
                    if len(result["planned_shifts"]) > 10:
                        self.stdout.write(
                            f"    ... and {len(result['planned_shifts']) - 10} more",
                        )
            else:
                ...
                self.stdout.write(
                    f"  Total shifts created: {result.get('total_shifts', 0)}",
                )
                self.stdout.write(
                    f"  Incident shifts: {result.get('incident_shifts', 0)}",
                )
                ...
                if created_shifts:
                    ...
                    for shift in created_shifts[:5]:  # Show first 5
                        engineer = (
                            shift.assigned_employee.user.username
                            if shift.assigned_employee
                            else "Unassigned"
                        )
                        self.stdout.write(
                            f"    - {shift.template.shift_type}: {engineer} ({shift.start_datetime.date()})",
                        )
