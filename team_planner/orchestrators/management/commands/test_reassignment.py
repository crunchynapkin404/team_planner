...
from team_planner.orchestrators.reassignment import ShiftReassignmentManager
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write(self.style.SUCCESS('🚀 Orchestrator Reassignment Test Command'))
        ...
        self.stdout.write(f"📅 Testing period: {start_date.date()} to {end_date.date()}")
        ...
        try:
            ...
            if options['dry_run']:
                ...
                self.stdout.write(self.style.WARNING("📋 DRY RUN - No shifts were actually created"))
            else:
            ...
            # Update run status
            run.status = OrchestrationRun.Status.COMPLETED if not options['dry_run'] else OrchestrationRun.Status.PREVIEW
            ...
            raise CommandError(f'Orchestration failed: {str(e)}')
    ...
    def _create_test_data(self):
        ...
        if profile_created:
            self.stdout.write(f"✅ Created employee profile with shift availability")
        else:
            ...
            profile.save()
            self.stdout.write(f"📋 Updated employee profile availability")
    ...
    def _get_orchestration_user(self):
        ...
        raise CommandError('No users found. Please create at least one user.')
    ...
    def _display_results(self, result):
        ...
        self.stdout.write(f"Average fairness score: {result.get('average_fairness', 0):.2f}")
        ...
        if reassignment_summary:
            ...
            self.stdout.write(f"Total conflicts detected: {reassignment_summary['total_conflicts_detected']}")
            self.stdout.write(f"Successful reassignments: {reassignment_summary['successful_reassignments']}")
            self.stdout.write(f"Split coverage reassignments: {reassignment_summary.get('split_coverage_reassignments', 0)}")
            self.stdout.write(f"Failed reassignments: {reassignment_summary['failed_reassignments']}")
            self.stdout.write(f"Manual interventions required: {reassignment_summary['manual_interventions_required']}")
            
            if reassignment_summary['total_conflicts_detected'] > 0:
                self.stdout.write('\\n📋 Conflicts by type:')
                for conflict_type, count in reassignment_summary['conflicts_by_type'].items():
                    if count > 0:
                        self.stdout.write(f"  - {conflict_type}: {count}")
            
            if reassignment_summary['reassignments']:
                self.stdout.write('\\n🔄 Reassignment details:')
                for i, reassignment in enumerate(reassignment_summary['reassignments'], 1):
                    original_emp = reassignment['original_employee']
                    new_emp = reassignment.get('new_employee')
                    status = self.style.SUCCESS("✅ SUCCESS") if reassignment['success'] else self.style.ERROR("❌ FAILED")
                    ...
                    if strategy == 'split_coverage':
                        ...
                        self.stdout.write(f"     Split Coverage: {split_info.get('kept_days', 0)} days kept, {split_info.get('reassigned_days', 0)} days to {new_emp.username if new_emp else 'unknown'}")
                    else:
                        self.stdout.write(f"     To: {new_emp.username if new_emp else 'Manual intervention required'}")
                    
                    self.stdout.write(f"     Reason: {reassignment['reason']}")
                    self.stdout.write(f"     Conflicts resolved: {len(reassignment['conflicts_resolved'])}")
        else:
            self.stdout.write('\\n' + self.style.SUCCESS('✅ No conflicts detected - no reassignments needed!'))
        
        # Display duplicate information if available
        if 'potential_duplicates' in result and result['potential_duplicates']:
            self.stdout.write('\\n' + self.style.WARNING('⚠️  POTENTIAL DUPLICATES'))
            self.stdout.write(f"Found {len(result['potential_duplicates'])} potential duplicate shifts")
        
        if 'skipped_duplicates' in result and result['skipped_duplicates']:
            self.stdout.write('\\n' + self.style.WARNING('⚠️  SKIPPED DUPLICATES'))
            self.stdout.write(f"Skipped {len(result['skipped_duplicates'])} duplicate shifts during creation")
