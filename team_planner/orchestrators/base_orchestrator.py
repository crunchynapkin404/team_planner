...
from team_planner.shifts.models import Shift, ShiftType, ShiftTemplate
from team_planner.employees.models import EmployeeProfile
from team_planner.leaves.models import LeaveRequest
from team_planner.orchestrators.models import OrchestrationRun, OrchestrationResult
...
class BaseOrchestrator(ABC):
    ...
    def __init__(self, team_id: Optional[int] = None):
        ...
        self.fairness_calculator: Optional[BaseFairnessCalculator] = None
        self._generation_assignment_count = 0  # Track assignments during current generation
        
        # Configurable parameters
        self.max_consecutive_weeks = int(getattr(settings, 'ORCHESTRATOR_MAX_CONSECUTIVE_WEEKS', 2))
        self.min_rest_hours = int(getattr(settings, 'ORCHESTRATOR_MIN_REST_HOURS', 48))
        self.continuity_preference_weight = float(getattr(settings, 'ORCHESTRATOR_CONTINUITY_WEIGHT', 1.5))
    ...
    @abstractmethod
    def _create_fairness_calculator(self, start_date: datetime, end_date: datetime) -> BaseFairnessCalculator:
    ...
    def _has_recurring_leave_conflict(self, employee: User, date: datetime) -> bool:
        ...
        try:
            ...
            for pattern in patterns:
                if pattern.applies_to_date(date.date()):
                    return True
    ...
    def generate_assignments(self, start_date: datetime, end_date: datetime, 
        ...
        last_assigned_employee: Dict[str, User] = {}  # Track last assigned per shift type
        
        while current_date <= end_date:
            # Generate assignments for this specific date
            daily_assignments = self._generate_assignments_for_date(
                current_date, employees, shift_templates, last_assigned_employee, assignments
            )
        ...
        return {
            ...
            'employees_assigned': len(set(a['assigned_employee_id'] for a in assignments)),
            'errors': [],
            'fairness_metrics': fairness_metrics
        }

    def _generate_day_by_day_assignments(self, start_date: datetime, end_date: datetime,
                                       employees: List[User], 
                                       shift_templates: List[ShiftTemplate]) -> List[Dict]:
    ...
    def _generate_assignments_for_date(self, date: datetime, employees: List[User],
        ...
        for template in shift_templates:
            ...
            assigned_employee = self._find_best_employee_for_shift(
                date, template, employees, last_assigned.get(shift_type), working_assignments
            )
    ...
    def _find_best_employee_for_shift(self, date: datetime, template: ShiftTemplate,
        ...
        selected_employee = self._select_employee_by_fairness(available_employees, template.shift_type, existing_assignments)
        ...
        if (last_assigned and last_assigned in available_employees and 
            template.shift_type not in ['incidents'] and selected_employee):
            
            # Get fairness scores for comparison
            current_assignments = self.fairness_calculator.calculate_current_assignments(available_employees) if self.fairness_calculator else {}
            if existing_assignments:
                provisional_assignments = self.fairness_calculator.calculate_provisional_assignments(existing_assignments) if self.fairness_calculator else {}
                ...
                for emp_id, provisional_data in provisional_assignments.items():
                    if emp_id in current_assignments:
                        for key, value in provisional_data.items():
                            if key in current_assignments[emp_id] and isinstance(value, (int, float)):
                                current_assignments[emp_id][key] += value
                    else:
                        current_assignments[emp_id] = provisional_data
            
            fairness_scores = self.fairness_calculator.calculate_fairness_score(current_assignments) if self.fairness_calculator else {}
    ...
    def _select_employee_by_fairness(self, employees: List[User], shift_type: str, 
                                   existing_assignments: Optional[List[Dict]] = None) -> Optional[User]:
        ...
        if existing_assignments:
            ...
            current_assignments = self.fairness_calculator.calculate_current_assignments(employees)
            
            # Add provisional assignments from current generation
            provisional_assignments = self.fairness_calculator.calculate_provisional_assignments(existing_assignments)
    ...
    def _calculate_assignment_fairness(self, assignments: List[Dict], 
                                     employees: List[User]) -> Dict[str, Any]:
        ...
        provisional_assignments = self.fairness_calculator.calculate_provisional_assignments(
            in_memory_assignments
        )
    ...
    def _save_orchestration_results(self, start_date: datetime, end_date: datetime,
                                  assignments: List[Dict], fairness_metrics: Dict[str, Any]):
        ...
        try:
            with transaction.atomic():
                ...
                print(f"✅ Successfully saved {len(created_shifts)} shifts to database")
