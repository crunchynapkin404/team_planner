...
from team_planner.orchestrators.fairness import FairnessCalculatorFactory, BaseFairnessCalculator
from team_planner.orchestrators.constraints import ConstraintCheckerFactory, BaseConstraintChecker
...
class BaseOrchestrator(ABC):
    ...
    def __init__(self, start_date: datetime, end_date: datetime, team_id: Optional[int] = None):
        ...
        self.orchestration_run: Optional[OrchestrationRun] = None
    ...
    def generate_schedule(self, orchestration_run: Optional[OrchestrationRun] = None) -> Dict[str, Any]:
        """
        ...
        logger.info(f"Starting {self.__class__.__name__} for period {self.start_date} to {self.end_date}")
    ...
    def _apply_reassignment_if_needed(self, assignments: List[Dict]) -> List[Dict]:
        ...
        try:
            ...
            reassignment_manager = ShiftReassignmentManager(self.orchestration_run, self.fairness_calculator)
            ...
            if conflicts:
                ...
                reassignments = reassignment_manager.resolve_conflicts(conflicts)
                ...
                for assignment in assignments:
                    if assignment.get('is_split_assignment', False):
                        ...
                        if split_info:
                            daily_assignments = reassignment_manager.create_split_shift_assignments(assignment, split_info)
    ...
    def _get_employees_available_for_day(self, employees: List[Any], check_date) -> List[Any]:
    ...
    def _select_best_employee_for_day(self, available_employees: List[Any], 
                                    check_date, day_label: str) -> Any:
    ...
    def _create_day_assignment(self, employee: Any, start_datetime: datetime, 
                             end_datetime: datetime, label: str) -> Dict:
    ...
    def _get_shift_template(self, shift_type: str) -> Optional[ShiftTemplate]:
    ...
    def _create_result(self, assignments: List[Dict], employees: List[Any]) -> Dict[str, Any]:
        ...
        provisional_assignments = self.fairness_calculator.calculate_provisional_assignments(assignments)
    ...
    def apply_schedule(self, result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.info(f"Applied schedule: {len(created_shifts)} shifts created, {len(skipped_duplicates)} duplicates skipped")
    ...
    def _create_orchestration_result(self, assignment: Dict, shift: Shift) -> Optional[OrchestrationResult]:
