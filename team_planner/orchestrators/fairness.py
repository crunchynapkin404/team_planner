...
class BaseFairnessCalculator:
    ...
    def calculate_current_assignments(self, employees: List[Any]) -> Dict[int, Dict[str, float]]:
        ...
        shifts = self._get_base_queryset().filter(
            assigned_employee__in=employee_ids
        )
        
        # Initialize result structure
        assignments = {}
        for emp in employees:
            assignments[emp.pk] = {shift_type: 0.0 for shift_type in self._shift_types}
        
        # Calculate hours per employee per shift type
        for shift in shifts:
            if shift.assigned_employee.pk in assignments:
                duration_hours = (shift.end_datetime - shift.start_datetime).total_seconds() / 3600
                shift_type_key = shift.template.shift_type.lower()
                if shift_type_key in assignments[shift.assigned_employee.pk]:
                    assignments[shift.assigned_employee.pk][shift_type_key] += duration_hours
        
        return assignments
    
    def calculate_provisional_assignments(self, assignments_list: List[Dict]) -> Dict[int, Dict[str, float]]:
        """Calculate provisional assignments from a list of assignment dictionaries."""
        provisional = defaultdict(lambda: {shift_type: 0.0 for shift_type in self._shift_types})
    ...
    def calculate_fairness_score(self, assignments: Dict[int, Dict[str, float]]) -> Dict[int, float]:
        ...
        if not all_hours:
            return {emp_id: 0.0 for emp_id in assignments.keys()}
    ...
    def get_least_assigned_employee(self, employees: List[Any]) -> Optional[Any]:
        ...
        least_assigned_id = min(fairness_scores.keys(), key=lambda x: fairness_scores[x])
...
class IncidentsFairnessCalculator(BaseFairnessCalculator):
    ...
    def calculate_incidents_load(self, employees: List[Any]) -> Dict[int, float]:
        ...
        assignments = self.calculate_current_assignments(employees)
        return {emp_id: data.get('incidents', 0.0) for emp_id, data in assignments.items()}
    
    def get_next_incidents_candidate(self, employees: List[Any]) -> Optional[Any]:
        ...
        sorted_employees = sorted(employees, key=lambda emp: incidents_loads.get(emp.pk, 0.0))
...
class IncidentsStandbyFairnessCalculator(BaseFairnessCalculator):
    ...
    def calculate_standby_load(self, employees: List[Any]) -> Dict[int, float]:
        ...
        assignments = self.calculate_current_assignments(employees)
        return {emp_id: data.get('incidents_standby', 0.0) for emp_id, data in assignments.items()}
    
    def get_next_standby_candidate(self, employees: List[Any]) -> Optional[Any]:
        ...
        sorted_employees = sorted(employees, key=lambda emp: standby_loads.get(emp.pk, 0.0))
        return sorted_employees[0] if sorted_employees else None
...
class WaakdienstFairnessCalculator(BaseFairnessCalculator):
    ...
    def calculate_waakdienst_load(self, employees: List[Any]) -> Dict[int, float]:
        ...
        assignments = self.calculate_current_assignments(employees)
        return {emp_id: data.get('waakdienst', 0.0) for emp_id, data in assignments.items()}
    
    def get_next_waakdienst_candidate(self, employees: List[Any]) -> Optional[Any]:
        ...
        sorted_employees = sorted(employees, key=lambda emp: waakdienst_loads.get(emp.pk, 0.0))
        return sorted_employees[0] if sorted_employees else None
    ...
    def calculate_weekend_burden(self, employees: List[Any]) -> Dict[int, float]:
        ...
        for emp in employees:
            emp_weekend_shifts = [s for s in weekend_shifts if s.assigned_employee.pk == emp.pk]
...
class FairnessCalculatorFactory:
    ...
    @staticmethod
    def create_for_shift_type(shift_type: str, start_date: datetime, end_date: datetime) -> BaseFairnessCalculator:
    ...
    @staticmethod
    def create_incidents_calculator(start_date: datetime, end_date: datetime) -> IncidentsFairnessCalculator:
        """Create an IncidentsFairnessCalculator."""
        return IncidentsFairnessCalculator(start_date, end_date)
    
    @staticmethod
    def create_standby_calculator(start_date: datetime, end_date: datetime) -> IncidentsStandbyFairnessCalculator:
        """Create an IncidentsStandbyFairnessCalculator."""
        return IncidentsStandbyFairnessCalculator(start_date, end_date)
    
    @staticmethod
    def create_waakdienst_calculator(start_date: datetime, end_date: datetime) -> WaakdienstFairnessCalculator:
