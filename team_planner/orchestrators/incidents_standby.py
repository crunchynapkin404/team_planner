...
class IncidentsStandbyOrchestrator(BaseOrchestrator):
    ...
    def _create_fairness_calculator(self) -> BaseFairnessCalculator:
        ...
        return FairnessCalculatorFactory.create_standby_calculator(self.start_date, self.end_date)
    
    def _create_constraint_checker(self) -> BaseConstraintChecker:
        """Create incidents-standby constraint checker."""
        return ConstraintCheckerFactory.create_incidents_checker(self.start_date, self.end_date)
    ...
    def _generate_period_assignments(self, period_start: datetime, period_end: datetime, 
        ...
        for assignment in self.current_assignments:
            ...
            if emp_id and emp_id in [emp.pk for emp in available_employees] and shift_type == 'incidents_standby':
                if emp_id not in current_assignments:
                    current_assignments[emp_id] = {'incidents': 0.0, 'incidents_standby': 0.0, 'waakdienst': 0.0}
        ...
        for emp in available_employees:
            ...
            for day_start, day_end, day_label in daily_shifts:
                ...
                availability_result = self.constraint_checker.check_employee_availability(emp, day_start.date())
        ...
        if fully_available:
            ...
            def sort_key(emp):
                return current_assignments.get(emp.pk, {}).get('incidents_standby', 0.0) + new_assignments[emp.pk]['incidents_standby']
    ...
    def _assign_partial_week_with_coverage(self, daily_shifts, partially_available, 
        ...
        def availability_sort_key(emp):
            availability = employee_availability.get(emp.pk, {})
            fairness = current_assignments.get(emp.pk, {}).get('incidents_standby', 0.0) + new_assignments[emp.pk]['incidents_standby']
    ...
    def _find_day_coverage(self, start_datetime, end_datetime, label, all_employees,
        ...
        for emp in all_employees:
            availability_result = self.constraint_checker.check_employee_availability(emp, start_datetime.date())
            if availability_result.is_available:
