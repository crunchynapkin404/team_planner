...
class OrchestrationResultInline(admin.TabularInline):
    ...
    extra = 0
    readonly_fields = [
        "employee", "shift_type", "week_start_date", "week_end_date",
        "fairness_score_before", "fairness_score_after", "assignment_reason",
...
class OrchestrationConstraintInline(admin.TabularInline):
    ...
    extra = 0
    readonly_fields = [
        "constraint_type", "severity", "employee", "start_date", "end_date",
        "shift_type", "description", "violations_count"
    ]
    can_delete = False
    ...
    fieldsets = (
        ...
        (_("Execution Details"), {
            "fields": (
                ...
                "total_shifts_created", "incidents_shifts_created", "waakdienst_shifts_created"
            ),
            "classes": ("collapse",)
        }),
        (_("Logs"), {
...
class OrchestrationResultAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (_("Assignment Details"), {
            "fields": (
                "orchestration_run", "employee", "shift_type",
...
class OrchestrationConstraintAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (_("Constraint Details"), {
            "fields": (
                "orchestration_run", "constraint_type", "severity", "description"
            )
        }),
        (_("Scope"), {
            "fields": ("employee", "start_date", "end_date", "shift_type")
        }),
        (_("Status"), {
    ...
    def constraint_type_badge(self, obj):
        ...
        return format_html(
            '<span style="background-color: {}; color: white; padding: 2px 8px; '
            'border-radius: 4px; font-size: 12px;">{}</span>',
            color, obj.get_constraint_type_display()
        )
