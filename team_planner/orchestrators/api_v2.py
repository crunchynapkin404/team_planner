...
from .fairness_calculators import BaseFairnessCalculator as FairnessCalculator
    ...
    try:
        ...
        try:
            if department_id.isdigit():
                ...
                team = Team.objects.select_related('department').filter(
        ...
        with transaction.atomic():
            ...
            try:
                ...
                run.completed_at = timezone.now()
                run.save()
                logger.error(f"V2 Orchestration failed: {str(e)}", exc_info=True)
                return Response({
        ...
        logger.error(f"V2 API error: {str(e)}", exc_info=True)
        return Response({
            'error': str(e),
            'code': 'INTERNAL_ERROR'
        }, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    ...
    try:
        ...
        if department_id:
            try:
                ...
                team_ids = list(department.teams.values_list('pk', flat=True))
        ...
        while current_date <= end_date:
            day_coverage = {}
            day_required = len(shift_types)  # Simplified: assume each day needs all shift types
            day_covered = 0
            
            for shift_type in shift_types:
                shifts_for_day_type = shifts_query.filter(
                    start_datetime__date=current_date,
                    template__shift_type=shift_type
                ).count()
            ...
            day_percentage = (day_covered / day_required * 100) if day_required > 0 else 0
        ...
        for shift_type in coverage_data['shift_types']:
            ...
            st_data['coverage_percentage'] = (
                ...
                if st_data['required_days'] > 0 else 0
            )
        ...
        logger.error(f"V2 Coverage API error: {str(e)}", exc_info=True)
        return Response({
            'error': str(e),
            'code': 'COVERAGE_ERROR'
        }, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
def orchestrator_availability_v2(request):
    ...
    try:
        ...
        # Validate shift type
        valid_shift_types = ['incidents', 'waakdienst', 'incidents_standby']
        ...
        if department_id:
            try:
                department = Department.objects.get(pk=department_id)
                team_ids = list(department.teams.values_list('pk', flat=True))
                # Note: This assumes employees have a team relationship
                # For now, we'll include all employees
            except Department.DoesNotExist:
                pass
        
        employees = employees_query.select_related('user')
        
        # Check for conflicts (existing shifts, leave requests, etc.)
        ...
        unavailable_employee_ids = set(
            shift.assigned_employee.pk for shift in existing_shifts 
        ...
        logger.error(f"V2 Availability API error: {str(e)}", exc_info=True)
        return Response({
            'error': str(e),
...
def orchestrator_health_v2(request):
    ...
    try:
        ...
        overall_status = 'healthy' if db_status == 'healthy' and orchestrator_status == 'operational' else 'unhealthy'
        ...
        logger.error(f"V2 Health API error: {str(e)}", exc_info=True)
        return Response({
            'status': 'unhealthy',
            'error': str(e),
            'timestamp': timezone.now().isoformat()
...
def orchestrator_metrics_v2(request):
    ...
    try:
        ...
        if recent_runs:
            ...
            avg_execution_time = total_time / len(recent_runs)
        ...
        metrics_data = {
            ...
            'employee_metrics': {
                ...
                'incidents_eligibility_percentage': round(
                    (incidents_eligible / total_active_employees * 100) if total_active_employees > 0 else 0, 2
                ),
                'waakdienst_eligibility_percentage': round(
                    (waakdienst_eligible / total_active_employees * 100) if total_active_employees > 0 else 0, 2
                )
            },
            'shift_metrics': {
                'shifts_this_month': shifts_this_month,
                'shifts_per_day_average': round(shifts_this_month / today.day, 2) if today.day > 0 else 0
            },
            'system_metrics': {
                'version': getattr(settings, 'VERSION', '1.0.0'),
        ...
        logger.error(f"V2 Metrics API error: {str(e)}", exc_info=True)
        return Response({
            'error': str(e),
            'code': 'METRICS_ERROR'
        }, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
def reset_assignment_history_v2(request):
    ...
    """
    # Staff permission required
    if not (request.user.is_staff or request.user.has_perm('orchestrators.delete_orchestrationrun')):
        return Response({
    ...
    try:
        ...
        logger.info(f"Assignment history reset by {request.user.username}: "
        ...
        logger.error(f"V2 Reset History API error: {str(e)}", exc_info=True)
        return Response({
            'error': str(e),
            'code': 'RESET_ERROR'
        }, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
def assignment_history_preview_v2(request):
    ...
    try:
        ...
        shifts = Shift.objects.filter(**shift_filters).select_related('assigned_employee', 'template')
        ...
        logger.error(f"V2 Assignment History Preview API error: {str(e)}", exc_info=True)
        return Response({
            'error': str(e),
            'code': 'PREVIEW_ERROR'
        }, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
