...
from team_planner.leaves.models import LeaveRequest, Holiday  # include Holiday for skip logic
from team_planner.orchestrators.anchors import business_weeks, waakdienst_periods, get_team_tz
...
class FairnessCalculator:
    ...
    WEEKEND_WEIGHT = 1.2
    HOLIDAY_WEIGHT = 1.5
    MANUAL_OVERRIDE_MULTIPLIER = 0.8  # count less towards fairness when manually overridden
    ...
    def __init__(self, start_date: datetime, end_date: datetime,
        ...
        self.MANUAL_OVERRIDE_MULTIPLIER = manual_override_multiplier or self.MANUAL_OVERRIDE_MULTIPLIER
        ...
        self._holiday_exact_dates: Optional[set] = None
        self._holiday_recurring_md: Optional[set[tuple[int, int]]] = None

    # --- Holiday helpers for desirability weighting ---
    def _ensure_holiday_cache(self):
        if self._holiday_exact_dates is not None and self._holiday_recurring_md is not None:
            return
        range_start = (self.start_date - timedelta(days=self.HISTORY_WINDOW_DAYS)).date()
        range_end = self.end_date.date()
        qs = Holiday.objects.filter(
            models.Q(date__gte=range_start, date__lte=range_end) | models.Q(is_recurring=True)
        )
        exact: set = set()
    ...
    def _is_holiday(self, d) -> bool:
        ...
        assert self._holiday_exact_dates is not None and self._holiday_recurring_md is not None
        return (d in self._holiday_exact_dates) or ((d.month, d.day) in self._holiday_recurring_md)

    @staticmethod
    def _overlap_hours(bounds_start: datetime, bounds_end: datetime, start: datetime, end: datetime) -> float:
    ...
    def _apply_manual_override_multiplier(self, hours: float, auto_assigned: bool) -> float:
        if auto_assigned is False:
            return hours * self.MANUAL_OVERRIDE_MULTIPLIER
        return hours

    def calculate_current_assignments(self, employees: List[Any]) -> Dict[int, Dict[str, float]]:
        ...
        for employee in employees:
            ...
            for shift in period_qs:
                base_weighted = self._weighted_hours(shift.start_datetime, shift.end_datetime)
                adjusted = self._apply_manual_override_multiplier(base_weighted, shift.auto_assigned)
            ...
            for shift in history_qs:
                ...
                weighted = self._weighted_hours(shift.start_datetime, shift.end_datetime)
                weighted = self._apply_manual_override_multiplier(weighted, shift.auto_assigned)
    ...
    def calculate_employee_available_hours(self, employee: Any) -> Dict[str, float]:
        ...
        patterns = RecurringLeavePattern.objects.filter(
            ...
            models.Q(effective_until__isnull=True) | models.Q(effective_until__gte=self.start_date.date())
        )
        ...
        for pattern in patterns:
            ...
            if pattern.frequency == 'weekly':
                ...
                weekly_reduction += pattern_hours_per_occurrence / 2  # Average over 2 weeks
        
        available_hours = max(0, standard_week_hours - weekly_reduction)
        availability_percentage = (available_hours / standard_week_hours * 100) if standard_week_hours > 0 else 0
    ...
    def calculate_fairness_score(self, assignments: Dict[int, Dict[str, float]]) -> Dict[int, float]:
        ...
        try:
            ...
            employees_qs = User.objects.filter(pk__in=emp_ids).select_related('employee_profile')
            profiles_by_id: Dict[int, Optional[EmployeeProfile]] = {
        ...
        for emp_id, data in normalized.items():
            ...
            total_available_capacity += base_avail * active_fraction_by_emp.get(emp_id, 1.0)
        ...
        for employee_id, data in normalized.items():
            ...
            if total_available_capacity > 0:
                ...
                if expected_hours <= 0:
                    ...
                    deviation_ratio = abs(employee_assigned_hours - expected_hours) / expected_hours
    ...
    def calculate_provisional_assignments(self, in_memory_assignments: List[Dict]) -> Dict[int, Dict[str, float]]:
        ...
        for a in in_memory_assignments:
            ...
            emp = User.objects.get(pk=emp_id) if emp_id not in employees else employees[emp_id]
    ...
    def get_returning_employee_info(self, user: Any) -> Optional[Dict[str, Any]]:
...
class ConstraintChecker:
    ...
    def __init__(self, start_date: datetime, end_date: datetime, team_id: Optional[int] = None, orchestrator=None):
        self.start_date = start_date
        self.end_date = end_date
        self.team_id = team_id
        self.orchestrator = orchestrator
        # Configurable knobs
        self.max_consecutive_weeks: int = int(getattr(settings, 'ORCHESTRATOR_MAX_CONSECUTIVE_WEEKS', 2))
        self.min_rest_hours: int = int(getattr(settings, 'ORCHESTRATOR_MIN_REST_HOURS', 48))
    ...
    def get_available_employees(self, shift_type: str) -> List[Any]:
        ...
        ).select_related('employee_profile').prefetch_related('employee_profile__skills')
    ...
    def check_recurring_pattern_conflicts(self, employee: Any, start_date: datetime, end_date: datetime, shift_type: str) -> bool:
        ...
        logger.debug(f"Allowing assignment for {employee.username} - conflicts will be handled by reassignment")
        return False

    def get_partial_availability_for_week(self, employee: Any, week_start: datetime, week_end: datetime, shift_type: str) -> Dict[str, Any]:
        ...
        if shift_type == ShiftType.WAAKDIENST:
            ...
            if not self.check_leave_conflicts(employee, week_start, week_end) and \
               not self.check_existing_assignments(employee, week_start, week_end, shift_type):
        ...
        patterns = RecurringLeavePattern.objects.filter(
            ...
            models.Q(effective_until__isnull=True) | models.Q(effective_until__gte=week_start.date())
        )
        ...
        while current_date <= week_end.date() and current_date.weekday() < 5:
            ...
            if self.check_leave_conflicts(employee, 
        ...
        return {
            ...
            'availability_percentage': (total_available_hours / total_possible_hours * 100) if total_possible_hours > 0 else 0
        }
    
    def check_existing_assignments(self, employee: Any, start_date: datetime, end_date: datetime, shift_type: Optional[str] = None) -> bool:
        ...
        if shift_type in [ShiftType.INCIDENTS, ShiftType.INCIDENTS_STANDBY]:
            qs = qs.filter(template__shift_type__in=[ShiftType.INCIDENTS, ShiftType.INCIDENTS_STANDBY])
        elif shift_type == ShiftType.WAAKDIENST:
    ...
    def check_incidents_conflicts_for_week(self, employee: Any, week_start: datetime, week_end: datetime, 
                                         checking_shift_type: str, existing_assignments: List[Dict]) -> bool:
        """Check if employee already has incidents or incidents-standby assigned for this week."""
        if checking_shift_type not in [ShiftType.INCIDENTS, ShiftType.INCIDENTS_STANDBY]:
            return False
        
        # Check existing assignments from this orchestration run
        for assignment in existing_assignments:
            if (assignment['assigned_employee_id'] == employee.pk and 
                assignment['week_start_date'] == week_start.date() and
                assignment['shift_type'] in [ShiftType.INCIDENTS, ShiftType.INCIDENTS_STANDBY] and
                assignment['shift_type'] != checking_shift_type):
                logger.info(f"Preventing double incidents assignment: {employee.username} already has {assignment['shift_type']} for week {week_start.date()}")
                return True
        ...
        for shift in week_shifts:
            if (shift.template.shift_type in [ShiftType.INCIDENTS, ShiftType.INCIDENTS_STANDBY] and
                shift.template.shift_type != checking_shift_type):
                logger.info(f"Preventing double incidents assignment: {employee.username} already has existing {shift.template.shift_type} for week {week_start.date()}")
                return True
    ...
    def _has_shift_in_range(self, employee: Any, start_dt: datetime, end_dt: datetime, types: List[str]) -> bool:
    ...
    def check_waakdienst_back_to_back(self, employee: Any, week_start: datetime, week_end: datetime) -> bool:
        """Prevent back-to-back waakdienst weeks by checking the previous waakdienst period."""
        prev_week_start = week_start - timedelta(days=7)
        prev_week_end = week_end - timedelta(days=7)
        return self._has_shift_in_range(employee, prev_week_start, prev_week_end, [ShiftType.WAAKDIENST])

    def count_consecutive_weeks(self, employee: Any, shift_type: str, week_start: datetime, week_end: datetime) -> int:
    ...
    def violates_min_rest(self, employee: Any, start_date: datetime, end_date: datetime, shift_type: str) -> bool:
    ...
    def is_employee_available(self, employee: Any, start_date: datetime, end_date: datetime, shift_type: str) -> bool:
        ...
        if self.check_recurring_pattern_conflicts(employee, start_date, end_date, shift_type):
            return False
        ...
        if self.violates_min_rest(employee, start_date, end_date, shift_type):
            logger.info(f"Rest constraint violation: {employee.username} cannot take {shift_type} due to {self.min_rest_hours}h rest rule")
            return False
        ...
        if shift_type == ShiftType.WAAKDIENST:
            ...
            if self.check_waakdienst_back_to_back(employee, week_start, week_end):
                logger.info(f"Back-to-back waakdienst prevented for {employee.username} (week starting {week_start})")
                return False
            consec = self.count_consecutive_weeks(employee, ShiftType.WAAKDIENST, week_start, week_end)
            if consec >= max(1, self.max_consecutive_weeks - 1):
                logger.info(f"Max consecutive waakdienst weeks reached for {employee.username}: {consec} prior weeks")
                return False

        # Check for incidents/incidents-standby conflicts for the same week
        if hasattr(self, 'orchestrator') and self.orchestrator and shift_type in [ShiftType.INCIDENTS, ShiftType.INCIDENTS_STANDBY]:
            if self.orchestrator.check_incidents_conflicts_for_employee(employee, start_date, end_date, shift_type):
                return False
...
class ShiftOrchestrator:
    ...
    def __init__(self, start_date: datetime, end_date: datetime, team_id: Optional[int] = None, 
                 ...
                 schedule_waakdienst: bool = True, orchestration_run=None):
        self.start_date = start_date
        self.end_date = end_date
        self.team_id = team_id
        ...
        self.constraint_checker = ConstraintChecker(start_date, end_date, team_id=team_id, orchestrator=self)
        ...
        if orchestration_run:
            ...
            self.reassignment_manager = ShiftReassignmentManager(orchestration_run, self.fairness_calculator)
    ...
    def _ensure_holiday_cache(self):
        ...
        range_end = self.end_date.date()
        qs = Holiday.objects.filter(models.Q(date__gte=range_start, date__lte=range_end) | models.Q(is_recurring=True))
        exact = set()
        recurring_md: set[tuple[int, int]] = set()
        for h in qs:
            if h.is_recurring:
    ...
    def generate_waakdienst_weeks(self) -> List[Tuple[datetime, datetime, str]]:
        ...
        if not team:
            ...
            wednesday_1700 = timezone.make_aware(datetime.combine(wednesday.date(), time(17, 0)))
    ...
    def generate_daily_shifts_for_week(self, week_start: datetime, week_end: datetime, 
        ...
        if week_type == 'business_week':
            ...
            for day_num in range(5):  # Monday to Friday
                ...
                if team and getattr(team, 'incidents_skip_holidays', False) and shift_type in [ShiftType.INCIDENTS, ShiftType.INCIDENTS_STANDBY]:
            ...
            thu_evening = wed_start.replace(hour=17) + timedelta(days=1)  # Thursday 17:00
            ...
            tue_evening = wed_start.replace(hour=17) + timedelta(days=6)  # Tuesday 17:00
    ...
    def get_shift_template(self, shift_type: str) -> Optional[ShiftTemplate]:
        ...
        try:
            ...
            logger.error(f"Error getting shift template for {shift_type}: {e}")
            # Fallback to first available template
            template = ShiftTemplate.objects.filter(shift_type=shift_type).first()
            if not template:
                raise ValueError(f"No shift template found for {shift_type}")
    ...
    def assign_shifts_fairly(self, shift_type: str, weeks: List[Tuple[datetime, datetime, str]]) -> List[Dict]:
        ...
        def sort_key(emp):
            emp_assignments = current_assignments.get(emp.pk, {'incidents': 0.0, 'incidents_standby': 0.0, 'waakdienst': 0.0})
        ...
        for week_start, week_end, week_type in weeks:
            ...
            if shift_type in [ShiftType.INCIDENTS, ShiftType.INCIDENTS_STANDBY]:
                ...
                week_assignments = self.assign_incidents_week_with_patterns_and_conflicts(
                    week_start, week_end, week_type, shift_type, available_employees, 
                    current_assignments, new_assignments, daily_shifts, assignments
                )
                ...
                week_assignments = self.assign_full_week_normal(
                    week_start, week_end, week_type, shift_type, available_employees,
                    current_assignments, new_assignments, daily_shifts
                )
                assignments.extend(week_assignments)
        
        return assignments
    
    def assign_incidents_week_with_patterns_and_conflicts(self, week_start: datetime, week_end: datetime, 
                                          week_type: str, shift_type: str, available_employees: List[Any],
                                          current_assignments: Dict, new_assignments: Dict, 
                                          daily_shifts: List[Tuple], existing_assignments: List[Dict]) -> List[Dict]:
        ...
        for emp in available_employees:
            availability_info = self.constraint_checker.get_partial_availability_for_week(
        ...
        for emp_id, data in employee_availability.items():
            emp = data['employee']
            # Avoid assigning the same engineer to both incidents and incidents-standby in the same week
            try:
                if self.check_incidents_conflicts_for_employee(emp, week_start, week_end, shift_type):
                    logger.info(
        ...
        if fully_available and partially_available:
            ...
            if partial_availability >= 60 and partial_load <= full_load + 2:
                ...
                return self.assign_partial_week_with_coverage(
                    week_start, week_end, shift_type, [best_partial], 
                    available_employees, current_assignments, new_assignments, daily_shifts
                )
            else:
                # Assign full week to best full candidate
                assigned_employee = best_full['employee']
                return self.create_full_week_assignments(assigned_employee, daily_shifts, shift_type, new_assignments)
        ...
        # Strategy 2: Handle partial availability with smart reassignment
            ...
            return self.assign_partial_week_with_coverage(
                week_start, week_end, shift_type, partially_available, 
                available_employees, current_assignments, new_assignments, daily_shifts
            )
    ...
    def assign_partial_week_with_coverage(self, week_start: datetime, week_end: datetime,
                                        shift_type: str, partially_available: List[Dict],
                                        all_employees: List[Any], current_assignments: Dict,
                                        new_assignments: Dict, daily_shifts: List[Tuple]) -> List[Dict]:
        ...
        for day_start, day_end, day_label in daily_shifts:
            ...
            if not day_conflicts:
                ...
                assignments.extend(self.create_day_assignment(
                    primary_employee, day_start, day_end, day_label, shift_type, new_assignments
                ))
                logger.info(f"✅ Assigned {primary_employee.username} to {day_label} (no conflicts)")
            else:
                # Day has conflicts - track for reassignment
                conflicted_days.append((day_start, day_end, day_label, day_conflicts[0]))
                logger.info(f"⚠️ Day conflict for {primary_employee.username} on {day_label}, will reassign")
        ...
        for day_start, day_end, day_label, day_conflict in conflicted_days:
            ...
            if available_hours > 0:
                ...
                logger.info(f"🔄 Splitting {day_label}: {primary_employee.username} works {available_hours}h, finding coverage for rest")
                split_assignments = self.assign_split_day_with_coverage(
                    day_start, day_end, day_label, shift_type, primary_employee, 
                    day_conflict, all_employees, current_assignments, new_assignments
                )
                assignments.extend(split_assignments)
            else:
                # Full day conflict - reassign entire day to someone else
                logger.info(f"🔄 Reassigning {day_label}: finding replacement for {primary_employee.username}")
                coverage_assignments = self.find_day_coverage(
                    day_start, day_end, day_label, shift_type, all_employees,
                    current_assignments, new_assignments, exclude_employee=primary_employee
                )
        ...
        logger.info(f"📋 Week assignment result: {primary_employee.username} gets {assigned_days}/{len(daily_shifts)} days, {len(conflicted_days)} days reassigned")
        
        return assignments
    
    def assign_split_day_with_coverage(self, day_start: datetime, day_end: datetime, day_label: str,
                                     shift_type: str, primary_employee: Any, day_conflict: Dict,
                                     all_employees: List[Any], current_assignments: Dict,
                                     new_assignments: Dict) -> List[Dict]:
        ...
        for conflict_detail in day_conflict['conflicts']:
            if conflict_detail['type'] == 'recurring_pattern':
                ...
                if pattern.coverage_type == 'morning':
                    ...
                    coverage_assignments = self.find_day_coverage(
                        morning_start, morning_end, f"{day_label}_morning_coverage", shift_type,
                        all_employees, current_assignments, new_assignments, exclude_employee=primary_employee
                    )
                    assignments.extend(coverage_assignments)
                    ...
                    assignments.extend(self.create_day_assignment(
                        primary_employee, afternoon_start, afternoon_end, 
                        f"{day_label}_afternoon_primary", shift_type, new_assignments
                    ))
                    ...
                    morning_end = day_start.replace(hour=12, minute=0)
                    assignments.extend(self.create_day_assignment(
                        primary_employee, morning_start, morning_end,
                        f"{day_label}_morning_primary", shift_type, new_assignments
                    ))
                    ...
                    afternoon_end = day_end
                    coverage_assignments = self.find_day_coverage(
                        afternoon_start, afternoon_end, f"{day_label}_afternoon_coverage", shift_type,
                        all_employees, current_assignments, new_assignments, exclude_employee=primary_employee
                    )
                    assignments.extend(coverage_assignments)
    ...
    def find_day_coverage(self, start_datetime: datetime, end_datetime: datetime, label: str,
                         shift_type: str, all_employees: List[Any], current_assignments: Dict,
                         new_assignments: Dict, exclude_employee: Any = None) -> List[Dict]:
        ...
        for emp in all_employees:
            ...
            if self.constraint_checker.is_employee_available(emp, start_datetime, end_datetime, shift_type):
        ...
        coverage_employee = self.find_complementary_pattern_employee(
            candidate_employees, start_datetime, current_assignments, new_assignments, shift_type
        )
        
        if coverage_employee:
            logger.info(f"🔄 Found complementary pattern coverage: {coverage_employee.username} for {label}")
            return self.create_day_assignment(coverage_employee, start_datetime, end_datetime, label, shift_type, new_assignments)
        ...
        logger.info(f"🔄 Fairness coverage: {coverage_employee.username} for {label} (load: {emp_load:.1f}h)")
        
        return self.create_day_assignment(coverage_employee, start_datetime, end_datetime, label, shift_type, new_assignments)
    
    def find_complementary_pattern_employee(self, candidates: List[Any], target_datetime: datetime,
                                          current_assignments: Dict, new_assignments: Dict, 
                                          shift_type: str) -> Optional[Any]:
        ...
        for emp in candidates:
            ...
            has_pattern_conflict_today = any(
                pattern.day_of_week == target_day_of_week 
                for pattern in patterns
            )
    ...
    def create_day_assignment(self, employee: Any, start_datetime: datetime, end_datetime: datetime,
                            label: str, shift_type: str, new_assignments: Dict) -> List[Dict]:
        ...
        new_assignments[employee.pk][shift_type.lower()] = new_assignments[employee.pk].get(shift_type.lower(), 0.0) + duration_hours
        return [assignment]
    
    def create_full_week_assignments(self, employee: Any, daily_shifts: List[Tuple], 
                                   shift_type: str, new_assignments: Dict) -> List[Dict]:
        ...
        try:
            ...
            if self.check_incidents_conflicts_for_employee(employee, week_start, week_end, shift_type):
                logger.info(
                    f"Prevented double assignment: {employee.username} already has complementary {shift_type} in same week"
                )
                return []
        except Exception as e:
            logger.warning(f"Weekly conflict guard failed in create_full_week_assignments: {e}")
        ...
        for day_start, day_end, day_label in daily_shifts:
            ...
            assignments.append({
                ...
                'shift_type': shift_type,
                'week_start_date': daily_shifts[0][0].date() if daily_shifts else day_start.date(),
                'auto_assigned': True,
        ...
        new_assignments[employee.pk][shift_type.lower()] = new_assignments[employee.pk].get(shift_type.lower(), 0.0) + total_hours
        
        return assignments
    
    def assign_full_week_normal(self, week_start: datetime, week_end: datetime, week_type: str,
                              shift_type: str, available_employees: List[Any], current_assignments: Dict,
                              new_assignments: Dict, daily_shifts: List[Tuple]) -> List[Dict]:
    ...
    def orchestrate(self) -> Dict[str, Any]:
        ...
        if self.reassignment_manager:
            ...
            if conflicts:
                logger.info(f"Detected {len(conflicts)} conflicts, attempting automatic reassignment")
                reassignments = self.reassignment_manager.resolve_conflicts(conflicts)
                ...
                for assignment in all_assignments:
                    if assignment.get('is_split_assignment', False):
                        ...
                        if split_info:
                            daily_assignments = self.reassignment_manager.create_split_shift_assignments(assignment, split_info)
                            updated_assignments.extend(daily_assignments)
                            split_assignments_to_remove.append(assignment)
                            logger.info(f"Replaced split assignment with {len(daily_assignments)} daily assignments")
                    else:
                ...
                reassignment_summary = self.reassignment_manager.get_reassignment_summary()
                logger.info(f"Reassignment complete: {reassignment_summary['successful_reassignments']} successful, {reassignment_summary['failed_reassignments']} failed")
            else:
        ...
        all_employees = list(set([a['assigned_employee_id'] for a in all_assignments]))
        provisional_assignments = self.fairness_calculator.calculate_provisional_assignments(all_assignments) if all_assignments else {}
        fairness_scores = self.fairness_calculator.calculate_fairness_score(provisional_assignments)
        ...
        if reassignment_summary:
            logger.info(f"Conflicts resolved: {reassignment_summary['successful_reassignments']} reassignments, {reassignment_summary['manual_interventions_required']} manual interventions needed")
        ...
        try:
            ...
            final_assignments_db = self.fairness_calculator.calculate_current_assignments(employee_objects)
    ...
    def apply_schedule(self) -> Dict[str, Any]:
        ...
        for assignment in schedule['assignments']:
            ...
            template = ShiftTemplate.objects.get(pk=assignment['template_id']) if assignment['template_id'] else None
    ...
    def check_incidents_conflicts_for_employee(self, employee: Any, start_date: datetime, end_date: datetime, shift_type: str) -> bool:
        ...
        for assignment in self.current_run_assignments:
            if (assignment['assigned_employee_id'] == employee.pk and 
                ...
                if assignment['start_datetime'] < end_date and assignment['end_datetime'] > start_date:
                    logger.info(
        ...
        try:
            week_start = datetime.combine(req_week, time(0, 0, 0, tzinfo=getattr(start_date, 'tzinfo', None)))
        except Exception:
        ...
        for shift in week_shifts:
            if (shift.template.shift_type in [ShiftType.INCIDENTS, ShiftType.INCIDENTS_STANDBY] and
                shift.template.shift_type != shift_type):
                logger.info(
