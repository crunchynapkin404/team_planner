...
class ShiftReassignmentManager:
    ...
    def _get_employee_from_assignment(self, assignment: dict) -> Any:
        ...
        msg = "Assignment missing both 'assigned_employee_id' and 'assigned_employee'"
        raise KeyError(
            msg,
        )
    ...
    def detect_conflicts(self, assignments: list[dict]) -> list[dict]:
        ...
        for assignment in assignments:
            ...
            double_conflicts = self._check_double_assignment_conflicts(
                ...
                for pattern in patterns:
                    if pattern.applies_to_date(current_date):
                        ...
                        if affected_hours:
                            ...
                            if (
                                assignment_start < pattern_end
                                and pattern_start < assignment_end
                            ):
                                conflicts.append(
                                    {
    ...
    def _check_double_assignment_conflicts(
        self, assignment: dict, all_assignments: list[dict],
    ) -> list[dict]:
        ...
        for other_assignment in all_assignments:
            ...
            if (
                ...
                and other_employee.pk == employee.pk
                and other_assignment["shift_type"]
                in [ShiftType.INCIDENTS, ShiftType.INCIDENTS_STANDBY]
                and other_assignment["shift_type"] != shift_type
            ):
    ...
    def resolve_conflicts(self, conflicts: list[dict]) -> list[dict]:
        ...
        for assignment_id, assignment_conflicts in conflicts_by_assignment.items():
            ...
            reassignment = self._apply_resolution_strategy(
                assignment, assignment_conflicts, strategy,
            )
    ...
    def _determine_resolution_strategy(self, conflicts: list[dict]) -> str:
        ...
        has_recurring = any(
            c["type"] == ConflictType.RECURRING_LEAVE for c in conflicts
        )
        has_approved_leave = any(
            c["type"] == ConflictType.APPROVED_LEAVE for c in conflicts
        )
            ...
            if shift_type in [ShiftType.INCIDENTS, ShiftType.INCIDENTS_STANDBY]:
                ...
                while current_date <= end_date:
                    if current_date.weekday() < 5:  # Weekdays only
                        ...
                        for conflict in conflicts:
                            if (
                                conflict["type"] == ConflictType.RECURRING_LEAVE
                                and conflict.get("conflict_date") == current_date
                            ):
                                conflict_days += 1
                                break
                    current_date += timedelta(days=1)
                ...
                if available_days > 0:
                    logger.info(
                        f"Using split coverage: {available_days}/{total_weekdays} days available",
                    )
    ...
    def _apply_resolution_strategy(
        self, assignment: dict, conflicts: list[dict], strategy: str,
    ) -> dict | None:
        ...
        if strategy == ReassignmentStrategy.SPLIT_COVERAGE:
            return self._split_coverage_reassignment(assignment, conflicts)
        if strategy == ReassignmentStrategy.MANUAL_INTERVENTION:
            return self._escalate_to_manual(assignment, conflicts)

        return None

    def _reassign_to_next_best(
        self, assignment: dict, conflicts: list[dict],
    ) -> dict | None:
        ...
        logger.info(
            f"Attempting reassignment for {original_employee.username} due to conflicts: {[c['type'] for c in conflicts]}",
        )
        ...
        if not available_employees:
            logger.warning(
                f"No available employees found for reassignment of {shift_type} shift",
            )
            return self._escalate_to_manual(assignment, conflicts)
        ...
        if self.fairness_calculator:
            try:
                load_map = self.fairness_calculator.calculate_provisional_assignments(
                    self.current_plan_assignments,
                )
                def key(emp):
                    return load_map.get(emp.pk, {}).get(shift_type.lower(), 0.0)
            except Exception:
                current_assignments = (
                ...
                def key(emp):
                    return current_assignments.get(emp.pk, {}).get(
                                    shift_type.lower(), 0.0,
                                )
        ...
        assignment["assignment_reason"] = (
            f"Reassigned from {original_employee.username} due to conflicts"
        )
        ...
        logger.info(
            f"Successfully reassigned {shift_type} shift from {original_employee.username} to {new_employee.username}",
        )

        return reassignment

    def _split_coverage_reassignment(
        self, assignment: dict, conflicts: list[dict],
    ) -> dict | None:
        ...
        logger.info(
            f"Attempting split coverage for {original_employee.username} with {len(conflicts)} conflicts",
        )
        ...
        if shift_type in [ShiftType.INCIDENTS, ShiftType.INCIDENTS_STANDBY]:
            return self._split_incidents_week_coverage(
                assignment, conflicts, conflict_dates,
            )

        # For other shift types, fall back to full reassignment
        logger.info(
            f"Split coverage not supported for {shift_type}, falling back to full reassignment",
        )
        return self._reassign_to_next_best(assignment, conflicts)

    def _split_incidents_week_coverage(
        self, assignment: dict, conflicts: list[dict], conflict_dates: set,
    ) -> dict | None:
        ...
        if not available_days:
            ...
            logger.warning(
                f"No available days for {original_employee.username}, full reassignment needed",
            )
            return self._reassign_to_next_best(assignment, conflicts)

        logger.info(
            f"Employee {original_employee.username} available for {len(available_days)}/{len(all_weekdays)} days",
        )
        ...
        replacement_employee = self._find_replacement_for_days(
            shift_type, conflict_days, original_employee,
        )

        if not replacement_employee:
            logger.warning(
                "No replacement found for conflict days, escalating to manual intervention",
            )
            return self._escalate_to_manual(assignment, conflicts)
        ...
        assignment["assignment_reason"] = (
            f"Split coverage: {len(available_days)} days for {original_employee.username}, {len(conflict_days)} days reassigned to {replacement_employee.username}"
        )
        ...
        logger.info(
            f"Successfully created split coverage: {original_employee.username} keeps {len(available_days)} days, {replacement_employee.username} covers {len(conflict_days)} days",
        )

        return reassignment

    def _find_replacement_for_days(
        self, shift_type: str, conflict_days: list, exclude_employee: Any,
    ) -> Any | None:
        ...
        for emp in all_available:
            ...
            for conflict_day in conflict_days:
                day_start = timezone.make_aware(
                    datetime.combine(conflict_day, time(8, 0)),
                )
                day_end = timezone.make_aware(
                    datetime.combine(conflict_day, time(17, 0)),
                )
                ...
                if constraint_checker.check_recurring_pattern_conflicts(
                    emp, day_start, day_end, shift_type,
                ):
                    available_for_all_days = False
                    break

                # Check for existing assignments on this specific day
                if constraint_checker.check_existing_assignments(
                    emp, day_start, day_end, shift_type,
                ):
                    available_for_all_days = False
                    break
        ...
        if self.fairness_calculator:
            try:
                ...
                suitable_employees.sort(
                    key=lambda emp: load_map.get(emp.pk, {}).get(
                        shift_type.lower(), 0.0,
                    ),
                )
            except Exception:
                ...
                suitable_employees.sort(
                    key=lambda emp: current_assignments.get(emp.pk, {}).get(
                        shift_type.lower(), 0.0,
                    ),
                )
    ...
    def create_split_shift_assignments(
        self, original_assignment: dict, split_info: dict,
    ) -> list[dict]:
        ...
        for day_date in split_info["original_employee_days"]:
            ...
            assignment = {
                ...
                "auto_assigned": True,
                "assignment_reason": "Split coverage - original employee available",
        ...
        logger.info(
            f"Created {len(assignments)} split shift assignments: {len(split_info['original_employee_days'])} for {original_employee.username}, {len(split_info['replacement_days'])} for {replacement_employee.username}",
        )
    ...
    def _escalate_to_manual(self, assignment: dict, conflicts: list[dict]) -> dict:
        ...
        reassignment = {
            ...
            "timestamp": timezone.now(),
            "reason": "Escalated to manual intervention - no automatic resolution available",
            "success": False,
        ...
        logger.warning(
            f"Escalating shift assignment to manual intervention: {assignment['shift_type']} for {original_employee.username}",
        )

        return reassignment

    def _get_available_employees_for_reassignment(
        self,
        shift_type: str,
        start_datetime: datetime,
        end_datetime: datetime,
        exclude_employee: Any,
    ) -> list[Any]:
        ...
        for emp in all_available:
            ...
            if not constraint_checker.check_leave_conflicts(
                emp, start_datetime, end_datetime,
            ):
        ...
        with transaction.atomic():
            for conflict in conflicts:
                OrchestrationConstraint.objects.create(
                    ...
                    employee=employee,
                    start_date=conflict.get(
                        "conflict_date", self.orchestration_run.start_date,
                    ),
                    end_date=conflict.get(
                        "conflict_date", self.orchestration_run.start_date,
                    ),
    ...
    def get_reassignment_summary(self) -> dict[str, Any]:
        ...
        split_coverage_reassignments = [
            r
            for r in self.reassignment_log
            if r.get("strategy") == ReassignmentStrategy.SPLIT_COVERAGE
        ]
        ...
        return {
            ...
            "manual_interventions_required": len(
                [
                    r
                    for r in self.reassignment_log
                    if r.get("requires_manual_action", False)
                ],
            ),
            "conflicts_by_type": {
                ConflictType.RECURRING_LEAVE: len(
                    [
                        c
                        for c in self.conflicts_detected
                        if c["type"] == ConflictType.RECURRING_LEAVE
                    ],
                ),
                ConflictType.APPROVED_LEAVE: len(
                    [
                        c
                        for c in self.conflicts_detected
                        if c["type"] == ConflictType.APPROVED_LEAVE
                    ],
                ),
                ConflictType.DOUBLE_ASSIGNMENT: len(
                    [
                        c
                        for c in self.conflicts_detected
                        if c["type"] == ConflictType.DOUBLE_ASSIGNMENT
                    ],
                ),
            },
    ...
    def validate_reassignment(
        ...
        if leave_requests.exists():
            ...
            validation_result["errors"].append(
                f"New employee {new_employee.username} has approved leave during assignment period",
            )
