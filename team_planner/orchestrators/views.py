...
from .fairness_calculators import (
    BaseFairnessCalculator as FairnessCalculator,
...
def create_orchestration(request):
    ...
    if request.method == "POST":
        try:
            ...
            schedule_incidents_standby = (
                request.POST.get("schedule_incidents_standby") == "on"
            )
            ...
            with transaction.atomic():
                ...
                try:
                    ...
                    if preview_only:
                        ...
                        if schedule_incidents:
                            shift_summary.append(
                                f"{result['incidents_shifts']} incidents",
                            )
                        if schedule_incidents_standby:
                            shift_summary.append(
                                f"{result['incidents_standby_shifts']} incidents-standby",
                            )
                        if schedule_waakdienst:
                            shift_summary.append(
                                f"{result['waakdienst_shifts']} waakdienst",
                            )

                        messages.success(
                            request,
                            f"Preview generated: {result['total_shifts']} daily shifts planned ({', '.join(shift_summary)})",
                        )
                    ...
                    run.incidents_standby_shifts_created = result[
                        "incidents_standby_shifts"
                    ]
                    ...
                    for shift in result.get("created_shifts", []):
                        OrchestrationResult.objects.create(
                            ...
                            shift=shift,
                            assignment_reason=shift.assignment_reason
                            or "Fair distribution algorithm",
                        )
            ...
            messages.error(request, f"Orchestration failed: {e!s}")
            return redirect("orchestrators:dashboard")
...
def orchestration_detail(request, pk):
    ...
    results = OrchestrationResult.objects.filter(orchestration_run=run).order_by(
        "week_start_date", "shift_type",
    )
    ...
    context = {
        ...
        "can_apply": (
            run.status == OrchestrationRun.Status.PREVIEW
            and not OrchestrationResult.objects.filter(
                orchestration_run=run, is_applied=True,
            ).exists()
        ),
    }
...
def apply_orchestration(request, pk):
    ...
    if run.status != OrchestrationRun.Status.PREVIEW:
        return JsonResponse(
            {"error": "Can only apply preview orchestrations"}, status=400,
        )

    if OrchestrationResult.objects.filter(
        orchestration_run=run, is_applied=True,
    ).exists():
            ...
            for result in OrchestrationResult.objects.filter(orchestration_run=run):
                if result.shift_type == "incidents":
                    ...
                    end_datetime = timezone.make_aware(
                        datetime.combine(
                            result.week_end_date - timedelta(days=2),
...
def fairness_report(request):
    ...
    if total_employees > 0:
        avg_incidents = (
            sum(d["incidents_count"] for d in fairness_data) / total_employees
        )
        avg_waakdienst = (
            sum(d["waakdienst_count"] for d in fairness_data) / total_employees
        )
...
def orchestration_history(request):
    ...
    runs = OrchestrationRun.objects.select_related("initiated_by").order_by(
        "-started_at",
    )

    return render(request, "orchestrators/history.html", {"runs": runs})
...
def fairness_dashboard(request):
    ...
    for employee in active_employees:
        emp_assignments = assignments.get(
            employee.pk, {"incidents": 0, "waakdienst": 0},
        )
        ...
        employee_data.append(
            {
                "name": (
                    getattr(employee, "get_full_name", lambda: "")()
                    or getattr(employee, "username", str(employee.pk))
                ),
...
def fairness_api(request):
    ...
    data = {"employees": [], "incidents": [], "waakdienst": [], "fairness_scores": []}
...
def run_horizon_now(request):
    ...
    try:
        ...
        summary = extend_rolling_horizon_core(
            months=months, dry_run=dry_run, team_ids=team_ids,
        )
        return render(
            request,
            "orchestrators/automation_result.html",
            {
                "summary": summary,
                "dry_run": dry_run,
                "months": months,
