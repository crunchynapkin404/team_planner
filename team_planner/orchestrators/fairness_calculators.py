...
from team_planner.leaves.models import LeaveRequest, Holiday
...
class BaseFairnessCalculator:
    ...
    def _get_tracked_shift_types(self) -> List[str]:
        ...
        """
        raise NotImplementedError("Subclasses must implement _get_tracked_shift_types")
    ...
    def calculate_fairness_score(self, assignments: Dict[int, Dict[str, float]]) -> Dict[int, float]:
        ...
        for emp_id, data in assignments.items():
            ...
            if 'total_hours' not in norm:
                ...
                tracked_types = self._get_tracked_shift_types()
                for shift_type in tracked_types:
                    if shift_type in norm and isinstance(norm[shift_type], (int, float)):
    ...
    def calculate_provisional_assignments(self, in_memory_assignments: List[Dict]) -> Dict[int, Dict[str, float]]:
        ...
        """
        assignments: Dict[int, Dict[str, float]] = defaultdict(lambda: {'total_hours': 0.0})
        tracked_types = self._get_tracked_shift_types()
        
        # Initialize tracked shift types
        for emp_id in assignments.keys():
            for shift_type in tracked_types:
