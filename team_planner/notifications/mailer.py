...
class IcsEvent:
    ...
    def to_ical(self) -> bytes:
        ...
        cal.add('version', '2.0')
        event = Event()
        event.add('uid', self.uid or f"team-planner-{uuid.uuid4()}@{getattr(settings, 'SITE_DOMAIN', 'example.com')}")
...
def send_email_with_optional_ics(
    ...
    *,
    ...
    if not to_list:
        return
    msg = EmailMultiAlternatives(subject=subject, body=body_text, to=to_list, from_email=from_email)
    if body_html:
...
def build_ics_for_leave(leave_request) -> IcsEvent:
    ...
    start_dt = timezone.make_aware(datetime.combine(leave_request.start_date, time(8, 0)), tz)
    end_dt = timezone.make_aware(datetime.combine(leave_request.end_date, time(17, 0)), tz)
...
def notify_swap_approved(requester_email: Optional[str], target_email: Optional[str], shift_summary: str, ics_event: Optional[IcsEvent] = None):
    subject = "Swap approved"
    body = f"Your swap request has been approved. {shift_summary}"
    if requester_email:
        send_email_with_optional_ics(subject, body, recipients=[requester_email], ics_event=ics_event)
    # Notify target as well
    body_target = f"You have been assigned a swap. {shift_summary}"
    if target_email:
        send_email_with_optional_ics(subject, body_target, recipients=[target_email], ics_event=ics_event)


def notify_leave_approved(employee_email: Optional[str], ics_event: Optional[IcsEvent]):
    subject = "Leave approved"
    body = "Your leave request has been approved. An event has been attached."
    if employee_email:
        send_email_with_optional_ics(subject, body, recipients=[employee_email], ics_event=ics_event)
