...
class LeaveRequest:
    ...
    def conflicts_with_time_range(self, time_range: TimeRange) -> bool:
        ...
        dates_overlap = (
            self.start_date <= shift_end_date and self.end_date >= shift_start_date
        )
...
class RecurringLeavePattern:
    ...
    def conflicts_with_time_range(self, time_range: TimeRange) -> bool:
        ...
        if self.coverage_type == "full_day":
            return True
        if self.coverage_type == "morning":
            return time_range.start.hour < 12
        if self.coverage_type == "afternoon":
            return time_range.start.hour >= 12
...
class Employee:
    ...
    def is_available_for_shift(
        self, shift_type: ShiftType, time_range: TimeRange,
    ) -> bool:
        ...
        if (
            ...
            and not self.available_for_incidents
        ):
            return False
    ...
    def has_conflict_with(self, assignment: "Assignment") -> Conflict | None:
    ...
    def _check_rest_period_conflict(
        ...
        for existing_assignment in self.current_assignments:
            ...
            time_between = abs(
                assignment.shift.time_range.start
                - existing_assignment.shift.time_range.end,
            )
    ...
    def _check_consecutive_weeks_conflict(
        self, assignment: "Assignment",
    ) -> Conflict | None:
        ...
        for existing_assignment in self.current_assignments:
            if (
                existing_assignment.shift
                and existing_assignment.shift.shift_type == assignment.shift.shift_type
            ):
                existing_week = (
                    existing_assignment.shift.time_range.start.date().isocalendar()[1]
                )
...
class Shift:
    ...
    # Assignment state
    assigned_employee: EmployeeId | None = None
    ...
    def assign_to(self, employee: Employee) -> "Assignment":
        ...
        if not employee.is_available_for_shift(self.shift_type, self.time_range):
            msg = f"Employee {employee.name} not available for this shift"
...
class Assignment:
    ...
    def calculate_impact(
        self, other_assignments: list["Assignment"],
    ) -> "AssignmentImpact":
