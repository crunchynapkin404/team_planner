...
class BusinessHoursConfiguration:
    ...
    monday_start: Optional[int] = 8
    monday_end: Optional[int] = 17
    tuesday_start: Optional[int] = 8
    tuesday_end: Optional[int] = 17
    wednesday_start: Optional[int] = 8
    wednesday_end: Optional[int] = 17
    thursday_start: Optional[int] = 8
    thursday_end: Optional[int] = 17
    friday_start: Optional[int] = 8
    friday_end: Optional[int] = 17
    saturday_start: Optional[int] = None  # No work by default
    saturday_end: Optional[int] = None
    sunday_start: Optional[int] = None    # No work by default
    sunday_end: Optional[int] = None
    ...
    def __post_init__(self):
        ...
        for day in ['monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday']:
            ...
            if start is not None and (start < 0 or start > 23):
                raise ValueError(f"{day}_start must be between 0 and 23")
            if end is not None and (end < 0 or end > 23):
                raise ValueError(f"{day}_end must be between 0 and 23")
            if start is not None and end is not None and start >= end:
                raise ValueError(f"{day}_start must be before {day}_end")

    def get_business_hours_for_day(self, day_of_week: int) -> Optional[Tuple[int, int]]:
...
class TeamConfiguration:
    ...
    def __post_init__(self):
        ...
        if not (0 <= self.waakdienst_start_day <= 6):
            raise ValueError("waakdienst_start_day must be between 0 and 6")
        if not (0 <= self.waakdienst_start_hour <= 23):
            raise ValueError("waakdienst_start_hour must be between 0 and 23")
        if not (0 <= self.waakdienst_end_hour <= 23):
            raise ValueError("waakdienst_end_hour must be between 0 and 23")
        
        # Validate constraint values
        if self.max_consecutive_weeks < 1:
            raise ValueError("max_consecutive_weeks must be at least 1")
        ...
        if self.fairness_period_days < 1:
            raise ValueError("fairness_period_days must be at least 1")
        if self.fairness_lookback_days < 1:
            raise ValueError("fairness_lookback_days must be at least 1")

    def get_business_hours_range(self, check_date: date) -> Optional[TimeRange]:
        ...
        start_datetime = datetime.combine(check_date, datetime.min.time().replace(hour=start_hour))
        end_datetime = datetime.combine(check_date, datetime.min.time().replace(hour=end_hour))
    ...
    def get_waakdienst_week_range(self, week_start: date) -> TimeRange:
        ...
        start_datetime = datetime.combine(
            wednesday, 
            datetime.min.time().replace(hour=self.waakdienst_start_hour)
        ).replace(tzinfo=tz)
        ...
        end_datetime = datetime.combine(
            next_wednesday,
            datetime.min.time().replace(hour=self.waakdienst_end_hour)
        ).replace(tzinfo=tz)
...
class AssignmentLoad:
    ...
    def __post_init__(self):
        expected_total = self.incidents_hours + self.incidents_standby_hours + self.waakdienst_hours
        if abs(self.total_hours - expected_total) > 0.01:  # Allow for floating point precision
            raise ValueError("Total hours must equal sum of individual shift type hours")
