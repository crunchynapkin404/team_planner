...
from ..value_objects import (
    EmployeeId, TeamId, DateRange, TimeRange, ShiftType,
    FairnessScore, TeamConfiguration, ConflictSeverity
)
from ..entities import Employee, Shift, Assignment, Conflict
...
class FairnessCalculator:
    ...
    def calculate_employee_fairness(
        ...
        fairness_assignments = [
            ...
            if fairness_period.contains(a.assigned_at.date())
        ]
        ...
        weighted_assignments = self._apply_historical_decay(
            fairness_assignments,
            period.start,
            self.config.fairness_lookback_days
        )
    ...
    def calculate_assignment_impact(
        ...
        if proposed_assignment.employee_id in current_scores:
            ...
            new_employee_score = current_scores[proposed_assignment.employee_id] + assignment_weight
            
            # Calculate impact
            employee_change = new_employee_score - current_scores[proposed_assignment.employee_id]
            
            # Calculate team variance before and after
            current_variance = statistics.variance(current_scores.values()) if len(current_scores) > 1 else 0
            
            scores_with_assignment = current_scores.copy()
            scores_with_assignment[proposed_assignment.employee_id] = new_employee_score
            new_variance = statistics.variance(scores_with_assignment.values()) if len(scores_with_assignment) > 1 else 0
    ...
    def find_fairest_employee(
        ...
        fairest_employee_id = min(candidate_scores.keys(), key=lambda emp_id: candidate_scores[emp_id])
    ...
    def _apply_historical_decay(
        ...
        for assignment in assignments:
            ...
            if days_ago <= lookback_days:
                ...
                weighted_assignments.append(WeightedAssignment(
                    assignment=assignment,
                    weight=decay_factor
                ))
    ...
    def _calculate_shift_type_score(
        self,
        weighted_assignments: List[WeightedAssignment],
...
class ConflictDetector:
    ...
    def detect_conflicts(
        ...
        conflicts.extend(self._check_rest_period_conflicts(assignment, existing_assignments))
        
        # Consecutive week conflicts
        conflicts.extend(self._check_consecutive_week_conflicts(assignment, existing_assignments))
        
        # Mutual exclusivity conflicts (incidents + waakdienst same week)
        conflicts.extend(self._check_mutual_exclusivity_conflicts(assignment, existing_assignments))
    ...
    def categorize_conflict_severity(self, conflict: Conflict) -> ConflictSeverity:
        ...
        blocking_types = {
            'leave_request', 'recurring_pattern', 'employee_unavailable',
            'rest_period', 'mutual_exclusivity'
        }
        ...
        warning_types = {
            'consecutive_weeks', 'fairness_impact'
        }
    ...
    def _check_leave_conflicts(self, assignment: Assignment, employee: Employee) -> List[Conflict]:
    ...
    def _check_recurring_pattern_conflicts(self, assignment: Assignment, employee: Employee) -> List[Conflict]:
    ...
    def _check_rest_period_conflicts(self, assignment: Assignment, existing_assignments: List[Assignment]) -> List[Conflict]:
    ...
    def _check_consecutive_week_conflicts(self, assignment: Assignment, existing_assignments: List[Assignment]) -> List[Conflict]:
        ...
        employee_assignments = [
            a for a in existing_assignments 
            if (a.employee_id == assignment.employee_id and 
                a.shift and 
                a.shift.shift_type == assignment.shift.shift_type)
        ]
    ...
    def _check_mutual_exclusivity_conflicts(self, assignment: Assignment, existing_assignments: List[Assignment]) -> List[Conflict]:
        ...
        assignment_week_end = assignment_week_start + timedelta(days=6)
        ...
        for existing in employee_assignments:
            ...
            if existing_week_start == assignment_week_start:
                ...
                if (assignment.shift.shift_type in [ShiftType.INCIDENTS, ShiftType.INCIDENTS_STANDBY] and
                    existing.shift.shift_type == ShiftType.WAAKDIENST):
                    conflicts.append(Conflict(
                ...
                elif (assignment.shift.shift_type == ShiftType.WAAKDIENST and
                      existing.shift.shift_type in [ShiftType.INCIDENTS, ShiftType.INCIDENTS_STANDBY]):
                    conflicts.append(Conflict(
                        conflict_type="mutual_exclusivity",
                        severity=ConflictSeverity.BLOCKING,
...
class ConstraintValidator:
    ...
    def validate_coverage_completeness(self, shifts: List[Shift]) -> 'CoverageValidation':
        ...
        for shift_date, date_shifts in shifts_by_date.items():
            ...
            if business_hours:
                incidents_coverage = any(
                    shift.shift_type in [ShiftType.INCIDENTS, ShiftType.INCIDENTS_STANDBY]
            ...
            has_waakdienst = any(
                shift.shift_type == ShiftType.WAAKDIENST
                for shift in date_shifts
            )
        ...
        return CoverageValidation(
            is_complete=len(coverage_gaps) == 0,
            gaps=coverage_gaps
        )
    
    def _validate_employee_availability(self, assignment: Assignment, employee: Employee) -> bool:
    ...
    def _validate_timezone_compatibility(self, assignment: Assignment, employee: Employee) -> bool:
    ...
    def _validate_team_membership(self, assignment: Assignment, employee: Employee) -> bool:
