...
# Domain imports
from domain.value_objects import DateRange, TimeRange, EmployeeId, ShiftType, TeamConfiguration, BusinessHoursConfiguration
...
class OrchestratorAPIViewSet(viewsets.ViewSet):
    ...
    async def orchestrate_schedule(self, request):
        ...
        try:
            ...
            shift_types = [ShiftType(st) for st in data.get('shift_types', ['incidents', 'waakdienst'])]
                ...
                'conflicts': [
                    {
                        ...
                        'affected_employee_id': conflict.employee_id.value if hasattr(conflict, 'employee_id') else None
                    }
    ...
    async def resolve_conflicts(self, request):
        ...
        try:
                ...
                'resolutions': [
                    {
                        ...
                        'old_employee_id': resolution.old_employee_id.value if resolution.old_employee_id else None,
            ...
            logger.error(f"Error in resolve_conflicts: {e}")
            return Response(
                {'error': 'Internal server error', 'message': str(e)},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
    ...
    async def validate_assignment(self, request):
        ...
        try:
            ...
            logger.error(f"Error in validate_assignment: {e}")
            return Response(
                {'error': 'Internal server error', 'message': str(e)},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
    ...
    async def fairness_report(self, request):
        ...
        try:
            ...
            logger.error(f"Error in fairness_report: {e}")
            return Response(
                {'error': 'Internal server error', 'message': str(e)},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
    ...
    async def employee_availability(self, request):
        ...
        try:
            ...
            # Create query
            query = GetEmployeeAvailabilityQuery(
