        ...
        try:
            async with self.uow:
                ...
                time_range_start = datetime.combine(
                    query.date_range.start, datetime.min.time(),
                )
                time_range_end = datetime.combine(
                    query.date_range.end, datetime.max.time(),
                )
                from domain.value_objects import TimeRange
                ...
                shift_query = ShiftQuery(
                    date_range=query_time_range, departments=query.department_ids,
                )
                ...
                schedule_data = {
                    ...
                    "summary": {
                        ...
                        "assigned_shifts": len(
                            [s for s in shifts if s.id in assignments_by_shift],
                        ),
                        "unassigned_shifts": len(
                            [s for s in shifts if s.id not in assignments_by_shift],
                        ),
            ...
            return QueryResult(
                success=False,
                message=f"Failed to retrieve schedule: {e!s}",
                errors=[str(e)],
            )
        ...
        try:
            async with self.uow:
                ...
                if query.include_assignments:
                    ...
                    assignments = (
                        await self.uow.assignments.find_by_employee_and_date_range(
                    ...
                    availability_data["assignments"] = [
                        {
                            ...
                            "start_time": assignment.shift.time_range.start.isoformat()
                            if assignment.shift
                            else None,
                            "end_time": assignment.shift.time_range.end.isoformat()
                            if assignment.shift
                            else None,
                            "status": assignment.status.value,
                        }
                ...
                if query.include_leave:
                    ...
                    leave_requests = (
                        await self.uow.leave_requests.find_by_employee_and_date_range(
                            query.employee_id, query_time_range,
                        )
                    )
            ...
            return QueryResult(
                success=False,
                message=f"Failed to retrieve employee availability: {e!s}",
                errors=[str(e)],
            )
    ...
    async def handle(self, query: GetUnassignedShiftsQuery) -> QueryResult:
        ...
        try:
            async with self.uow:
                ...
                unassigned_shifts = await self.uow.shifts.find_unassigned_in_range(
                    query_time_range,
                )
            ...
            return QueryResult(
                success=False,
                message=f"Failed to retrieve unassigned shifts: {e!s}",
                errors=[str(e)],
            )
        ...
        try:
            async with self.uow:
                ...
                employee_query = EmployeeQuery(
                    active_only=True, department_ids=query.department_ids,
                )
                ...
                for employee in employees:
                    ...
                    fairness_result = (
                        self.fairness_calculator.calculate_employee_fairness(
                ...
                # Calculate summary statistics
                if fairness_scores:
                    fairness_data["summary"]["average_fairness"] = sum(
                        fairness_scores,
                    ) / len(fairness_scores)
                    ...
                    if len(fairness_scores) > 1:
                        ...
                        fairness_data["summary"]["fairness_variance"] = (
                            statistics.variance(fairness_scores)
                        )
                    ...
                    for emp_fairness in fairness_data["employee_fairness"]:
                        if emp_fairness["total_score"] == min_score:
                            fairness_data["summary"]["most_fair"] = emp_fairness[
                                "employee_id"
                            ]
                        if emp_fairness["total_score"] == max_score:
                            fairness_data["summary"]["most_unfair"] = emp_fairness[
                                "employee_id"
                            ]
            ...
            return QueryResult(
                success=False,
                message=f"Failed to generate fairness report: {e!s}",
                errors=[str(e)],
            )
