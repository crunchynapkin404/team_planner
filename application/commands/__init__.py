    ...
    async def handle(self, command: CreateScheduleCommand) -> CommandResult:
        ...
        try:
            ...
            async with self.uow:
                ...
                from ..use_cases import OrchestrateScheduleUseCase, SchedulingRequest
                ...
                return CommandResult(
                    ...
                    data={
                        'assignments': [{'id': a.id, 'employee_id': a.employee_id} for a in result.assignments],
                        'unassigned_shifts': [{'id': s.id} for s in result.unassigned_shifts],
                        'conflicts': result.conflicts_detected,
                        'fairness_scores': {str(k): v for k, v in result.fairness_scores.items()}
                    },
...
class UnassignShiftCommandHandler(CommandHandler):
    ...
    async def handle(self, command: UnassignShiftCommand) -> CommandResult:
        ...
        try:
            ...
            async with self.uow:
                # Get assignment
                assignment = await self.uow.assignments.find_by_id(command.assignment_id)
                if not assignment:
                    return CommandResult(
                        success=False,
    ...
    async def handle(self, command: SwapAssignmentsCommand) -> CommandResult:
        ...
        try:
            ...
            async with self.uow:
                ...
                assignment1 = await self.uow.assignments.find_by_id(command.assignment_id_1)
                assignment2 = await self.uow.assignments.find_by_id(command.assignment_id_2)
                ...
                return CommandResult(
                    ...
                    data={
                        'swapped_assignments': [
                            {'assignment_id': assignment1.id, 'new_employee_id': assignment1.employee_id},
                            {'assignment_id': assignment2.id, 'new_employee_id': assignment2.employee_id}
                        ]
                    }
                )
            ...
            return CommandResult(
                success=False,
                message=f"Assignment swap failed: {str(e)}",
                errors=[str(e)]
            )
