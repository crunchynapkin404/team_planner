...
if TYPE_CHECKING:
    ...
    from domain.services import ConstraintValidator
...
class SchedulingRequest:
    ...
    force_assignments: dict["ShiftId", "EmployeeId"] | None = None
...
class OrchestrateScheduleUseCase:
        ...
        try:
            async with self.uow:
                ...
                remaining_shifts = [
                    ...
                    if not any(
                        hasattr(a, "shift_id") and a.shift_id == s.id
                ...
                final_conflicts = await self._detect_final_conflicts(
                    assignments, available_employees,
                )
                ...
                unassigned_shifts = [
                    s for s in shifts_to_assign if s.id not in assigned_shift_ids
                ]
            ...
            await self.uow.rollback()
            msg = f"Scheduling failed: {e!s}"
            raise SchedulingError(msg) from e
    ...
    async def _discover_shifts(self, request: SchedulingRequest) -> list["Shift"]:
        ...
        time_range_start = datetime.combine(
            request.date_range.start, datetime.min.time(),
        )
    ...
    async def _calculate_fairness_scores(
        self, employees: list["Employee"], date_range: "DateRange",
    ...
    async def _process_forced_assignments(
        self, request: SchedulingRequest, shifts: list["Shift"],
    ...
    async def _validate_coverage(
        self, assignments: list["Assignment"], date_range: "DateRange",
    ) -> dict[str, Any]:
    ...
    async def _detect_final_conflicts(
        self, assignments: list["Assignment"], employees: list["Employee"],
    ) -> list[dict[str, Any]]:
            ...
            for assignment_id in assignment_ids:
                ...
                if assignment:
                    ...
                    if assignment.employee_id not in employees:
                        employee = await self.uow.employees.find_by_id(
