...
from team_planner.employees.models import EmployeeProfile, EmployeeSkill, RecurringLeavePattern
...
from application.repositories import (
    ...
    LeaveRequestRepository, UnitOfWork,
    EmployeeQuery, ShiftQuery, AssignmentQuery
)

# Domain layer imports
from domain.entities import Employee, Shift as DomainShift, Assignment
from domain.value_objects import (
    TimeRange, ShiftType, EmployeeId, ShiftId, DateRange, TeamId,
    TeamConfiguration, BusinessHoursConfiguration, AssignmentStatus, AssignmentId
)
...
class DjangoEmployeeRepository(EmployeeRepository):
    ...
    async def find_by_id(self, employee_id: EmployeeId) -> Optional[Employee]:
        """Find employee by ID."""
        try:
            user = await sync_to_async(User.objects.select_related('employee_profile').get)(
    ...
    async def get_employee_assignments_count(
        ...
        count = await sync_to_async(Shift.objects.filter(
            assigned_employee_id=employee_id.value,
            start_datetime__gte=date_range.start,
            end_datetime__lte=date_range.end,
            status__in=[Shift.Status.SCHEDULED, Shift.Status.CONFIRMED, Shift.Status.IN_PROGRESS]
        ).count)()
        return count
    ...
    async def get_employees_by_department(self, department_id: str) -> List[Employee]:
        ...
        users = await sync_to_async(list)(
            User.objects.select_related('employee_profile').filter(
    ...
    def _map_to_domain_employee(self, user: Any) -> Employee:
        ...
        if not profile:
            raise ValueError(f"User {user.pk} has no employee profile")
        ...
        if team_memberships and hasattr(team_memberships, 'all'):
            ...
            if teams:
                ...
                business_hours = BusinessHoursConfiguration(
                    monday_start=8, monday_end=17,
                    tuesday_start=8, tuesday_end=17,
                    wednesday_start=8, wednesday_end=17,
                    thursday_start=8, thursday_end=17,
                    friday_start=8, friday_end=17,
        ...
        return Employee(
            ...
            name=getattr(user, 'get_full_name', lambda: getattr(user, 'username', 'Unknown'))() or getattr(user, 'username', 'Unknown'),
...
class DjangoShiftRepository(ShiftRepository):
    ...
    async def find_by_id(self, shift_id: ShiftId) -> Optional[DomainShift]:
        """Find shift by ID."""
        try:
            django_shift = await sync_to_async(Shift.objects.select_related(
                'template', 'assigned_employee'
            ).get)(id=shift_id.value)
            return self._map_to_domain_shift(django_shift)
        except Shift.DoesNotExist:
            return None
    
    async def find_all(self, query: Optional[ShiftQuery] = None) -> List[DomainShift]:
        ...
        if query:
            ...
            if query.shift_types:
                django_shift_types = [self._map_to_django_shift_type(st) for st in query.shift_types]
    ...
    async def find_unassigned_in_range(self, date_range: TimeRange) -> List[DomainShift]:
    ...
    async def save(self, shift: DomainShift) -> None:
        ...
        django_shift_data = {
            ...
            'status': shift.assignment_status.value if hasattr(shift.assignment_status, 'value') else str(shift.assignment_status),
        ...
        if shift.id:
            # Update existing
            await sync_to_async(Shift.objects.filter(id=shift.id.value).update)(**django_shift_data)
        else:
            # Create new
            django_shift = await sync_to_async(Shift.objects.create)(**django_shift_data)
    ...
    def _map_to_domain_shift(self, django_shift: Any) -> DomainShift:
        ...
        time_range = TimeRange(
            ...
            timezone=django_shift.start_datetime.tzinfo.zone if django_shift.start_datetime.tzinfo else 'UTC'
        )
        
        shift_type = self._map_to_domain_shift_type(django_shift.template.shift_type)
        
        # Get team ID from shift template or default
        team_id = TeamId(1)  # Default team - in real implementation would get from template
        ...
        return DomainShift(
            ...
            assignment_status=self._map_to_domain_assignment_status(django_shift.status),
    ...
    def _map_to_domain_shift_type(self, django_shift_type: str) -> ShiftType:
        ...
        elif django_shift_type == DjangoShiftType.INCIDENTS_STANDBY:
            return ShiftType.INCIDENTS_STANDBY
        elif django_shift_type == DjangoShiftType.WAAKDIENST:
    ...
    def _map_to_django_shift_type(self, domain_shift_type: ShiftType) -> str:
        ...
        elif domain_shift_type == ShiftType.INCIDENTS_STANDBY:
            return DjangoShiftType.INCIDENTS_STANDBY
        elif domain_shift_type == ShiftType.WAAKDIENST:
    ...
    def _map_to_django_status(self, domain_status: AssignmentStatus) -> str:
        ...
        if domain_status == AssignmentStatus.CONFIRMED:
            return 'confirmed'
        elif domain_status == AssignmentStatus.CANCELLED:
            return 'cancelled'
        else:
            return 'scheduled'
    ...
    async def _get_or_create_template(self, shift_type: ShiftType) -> ShiftTemplate:
        ...
        template, created = await sync_to_async(ShiftTemplate.objects.get_or_create)(
            ...
            defaults={
                ...
                'duration_hours': 9 if shift_type in [ShiftType.INCIDENTS, ShiftType.INCIDENTS_STANDBY] else 15
            }
        )
        return template
...
class DjangoAssignmentRepository(AssignmentRepository):
    ...
    async def find_all(self, query: Optional[AssignmentQuery] = None) -> List[Assignment]:
        ...
        queryset = Shift.objects.select_related('template', 'assigned_employee').exclude(
            assigned_employee__isnull=True
        )
    ...
    async def find_by_shift(self, shift_id: ShiftId) -> List[Assignment]:
        ...
        try:
            shift = await sync_to_async(Shift.objects.select_related('template', 'assigned_employee').get)(
                id=shift_id.value
            )
            if shift.assigned_employee:
    ...
    async def save(self, assignment: Assignment) -> None:
        ...
        if hasattr(assignment, 'shift_id') and assignment.shift_id:
            await sync_to_async(Shift.objects.filter(id=assignment.shift_id.value).update)(
    ...
    def _map_to_domain_assignment(self, django_shift: Any) -> Assignment:
        ...
        if not django_shift.assigned_employee:
            raise ValueError("Cannot create assignment from unassigned shift")
        ...
        return Assignment(
            ...
            status=AssignmentStatus.CONFIRMED if django_shift.status in [
...
class DjangoLeaveRequestRepository(LeaveRequestRepository):
    ...
    async def is_employee_on_leave(
        ...
        count = await sync_to_async(LeaveRequest.objects.filter(
            employee_id=employee_id.value,
