...
from domain.value_objects import (
    ...
    BusinessHoursConfiguration, TeamConfiguration
)
from domain.entities import (
    Employee, Shift, Assignment, Conflict, LeaveRequest,
    RecurringLeavePattern, ValidationResult
)
...
class TestRecurringLeavePattern:
    ...
    def test_biweekly_pattern_conflict(self):
        ...
        first_monday = TimeRange(
            datetime.combine(start_date, datetime.min.time().replace(hour=9, tzinfo=tz)),
            datetime.combine(start_date, datetime.min.time().replace(hour=17, tzinfo=tz)),
            'Europe/Amsterdam'
        )
        
        # Second Monday (week 1 - should not conflict)
        second_monday = TimeRange(
            datetime.combine(start_date + timedelta(weeks=1), datetime.min.time().replace(hour=9, tzinfo=tz)),
            datetime.combine(start_date + timedelta(weeks=1), datetime.min.time().replace(hour=17, tzinfo=tz)),
            'Europe/Amsterdam'
        )
        
        # Third Monday (week 2 - should conflict again)
        third_monday = TimeRange(
            datetime.combine(start_date + timedelta(weeks=2), datetime.min.time().replace(hour=9, tzinfo=tz)),
            datetime.combine(start_date + timedelta(weeks=2), datetime.min.time().replace(hour=17, tzinfo=tz)),
            'Europe/Amsterdam'
        )
...
class TestEmployee:
    ...
    def test_recurring_pattern_blocks_availability(self):
        ...
        if monday_shift.start.weekday() == 0:  # Ensure it's Monday
            assert not employee.is_available_for_shift(ShiftType.INCIDENTS, monday_shift)
